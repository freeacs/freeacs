...
public class ACSWSFactory {
	...
	public static ACSWS getXAPSWS(Login login, DataSource xaps, DataSource syslog) throws RemoteException {
		...
		if (cache.get(key) == null) {
			...
			ACSWS xapsWS = new ACSWS(login, lifetimeSec, xaps, syslog);
			cache.put(key, new CacheValue(xapsWS, Cache.ABSOLUTE, lifetimeSec * 1000));
		}
		return (ACSWS) cache.get(key).getObject();
	}

...
public class UnitJobResult {
  ...
  private boolean confirmedFailure = false;
  private boolean unconfirmedFailure = false;

  public UnitJobResult() {}

  /* The method sets the confirmed flag, but also
   * return -1, 0 or 1, depending upon these conditions:
   * 1: change to confirmedFailure
   * 0: no change
   * -1: change to !confirmedFailure
   */
  ...
  public int setUnconfirmed(int unconfirmed) {
    int result = 0;
    if (unconfirmed > 0) {
      if (!unconfirmedFailure) result = 1;

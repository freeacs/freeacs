...
public class FractionStopRuleCounter {
	...
	public FractionStopRuleCounter(StopRule rule) {
		if (rule.getNumberMax() == null)
			throw new IllegalArgumentException("This rule is not a fraction stop rule: " + rule.toString());
		this.rule = rule;
	...
	public boolean ruleMatch() {
		...
		if (failures >= rule.getNumberLimit())
			return true;
		else
			return false;
	}
	...
	public String toString() {
		return rule + " (confirmed failures:" + counters[CONFIRMED] + ", unconfirmed failures:" + counters[UNCONFIRMED] + ")";
	}

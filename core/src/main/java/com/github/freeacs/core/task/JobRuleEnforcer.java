...
public class JobRuleEnforcer extends DBIOwner {
  ...
  public class JobControl {
    ...
    public void updateRules(Job newJob) {
      ...
      for (StopRule newRule : newJob.getStopRules()) {
        if (newRule.getNumberMax() == null) continue; // only focus on Fraction Stop Rules
        boolean match = false;
        ...
        if (!match) {
          ...
          for (FractionStopRuleCounter fsrc : fractionStopRuleCounters) {
            if (fsrc.getRule().getRuleType() == newRule.getRuleType()) {
              if (preferred == null) preferred = fsrc;
      ...
      for (StopRule newRule : newJob.getStopRules()) {
        if (newRule.getNumberMax() != null) continue; // only focus on Absolute Rules
        boolean match = false;
        for (StopRule rule : job.getStopRules()) {
        ...
        if (match) logger.info("JobRuleEnforcer: \tKept absolute rule: " + newRule.toString());
        else logger.info("JobRuleEnforcer: \tAdded absolute rule: " + newRule.toString());
      }
      for (StopRule rule : job.getStopRules()) {
        if (rule.getNumberMax() != null) continue; // only focus on Absolute Rules
        boolean match = false;
        for (StopRule newRule : newJob.getStopRules()) {
          if (newRule.toString().equals(rule.toString())) {
            match = true;
        ...
        if (!match) logger.info("JobRuleEnforcer: \tDeleted absolute rule: " + rule.toString());
      }
      job = newJob;
          ...
          if (jcJob.getStopRulesSerialized() != null
            ...
            logger.info(
                "JobRuleEnforcer: Detected stop rules change in job "
                    ...
                    + ").");
            jc.updateRules(j);
          } else if ((jcJob.getStopRulesSerialized() != null && j.getStopRulesSerialized() == null)
  ...
  private void process() throws Exception {
    for (JobControl jc : jobControlMap.values()) {
      ...
      if (ujList.size() > 0) processUnprocessed(ujList, jc);
  ...
  private void processUnprocessed(List<UnitJob> ujList, JobControl jc) throws SQLException {
    ...
    for (UnitJob uj : ujList) {
      ...
      for (FractionStopRuleCounter fr : jc.getFractionStopRuleCounters()) fr.addResult(uj);
      unitJobs.markAsProcessed(uj);
      logger.info(
          "JobRuleEnforcer: ["
              + job.getName()
  ...
  private void updateJob(Job job, UnitJobs unitJobs) throws SQLException {
    ...
    if (confirmedFailedThisRound != 0) publishMsg += "cf=" + confirmedFailed + ",";
    ...
    if (unconfirmedFailedThisRound != 0) publishMsg += "uf=" + unconfirmedFailed + ",";

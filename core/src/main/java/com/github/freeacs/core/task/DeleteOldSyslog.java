...
public class DeleteOldSyslog extends DBIShare {
	...
	private void removeOldSyslogEntriesEventBased(List<SyslogEvent> events) throws SQLException {
		...
		for (SyslogEvent event : events) {
			...
			if (event.getDeleteLimit() != null && event.getDeleteLimit() > 0) {
				...
				int rowsDeleted = 0;
				int loopCounter = 0;
				logger.info("DeleteOldSyslog: Will delete all syslog rows older than " + limitCal.getTime() + " with eventId " + event.getEventId() + ", unittype " + event.getUnittype().getName());
				do {
					if (fromCal != null && toCal.getTime().getTime() - fromCal.getTime().getTime() >= 7 * 86400 * 1000) {
						logger.debug("DeleteOldSyslog: Period is now one week - setting from to null => ask for all remaining entries");
						fromCal = null;
					}
					rowsDeleted = getSyslog().deleteOldEventsEntries(fromCal, toCal, event, 500000);
					if (fromCal != null)
						logger.info("DeleteOldSyslog: " + rowsDeleted + " rows were deleted from the syslog table for the period " + fromCal.getTime() + " - " + toCal.getTime());
					else
						logger.info("DeleteOldSyslog: " +rowsDeleted + " rows were deleted from the syslog table from beginning - " + toCal.getTime());
					if (rowsDeleted == 500000) {
						...
						if (!"comprehensive".equals(Properties.SYSLOG_CLEANUP) && rowsDeleted == 0) {
							...
							if (loopCounter == 0) {
								...
								logger.debug("DeleteOldSyslog: Increasing the period by one day, since all entries were deleted on the first try");
							}
	...
	private void removeOldSyslogEntriesSeverityBased(List<SyslogEvent> events) throws SQLException {
		...
		for (int severity = 0; severity <= 7; severity++) {
			...
			String logMsg = "Will delete all syslog rows older than " + limitCal.getTime() + " with severity level " + severity;
			...
			do {
				if (fromCal != null && toCal.getTime().getTime() - fromCal.getTime().getTime() >= 7 * 86400 * 1000) {
					logger.debug("DeleteOldSyslog: Period is now one week - setting from-tms to null => ask for all remaining entries");
					fromCal = null;
				}
				rowsDeleted = getSyslog().deleteOldSeverityEntries(fromCal, toCal, severity, eventList, 500000);
				if (fromCal != null)
					logger.info("DeleteOldSyslog: " + rowsDeleted + " rows were deleted from the syslog table for the period " + fromCal.getTime() + " - " + toCal.getTime());
				else
					logger.info("DeleteOldSyslog: " + rowsDeleted + " rows were deleted from the syslog table from beginning - " + toCal.getTime());

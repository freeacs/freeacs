...
public class Sleep {
  ...
  public Sleep(int maxSleep, int minSleep, boolean sleepFirstTime) {
    ...
    if (minSleep > maxSleep) minSleep = maxSleep;
    if (sleepFirstTime) this.lastReturnTms = System.currentTimeMillis();
    else this.lastReturnTms = 0;
  }
  ...
  public void sleep() {
    ...
    if (suggestedSleepTime < minSleep) sleepTime = minSleep;
    else sleepTime = suggestedSleepTime;
    try {

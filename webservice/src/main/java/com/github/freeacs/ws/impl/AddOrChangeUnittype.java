...
public class AddOrChangeUnittype {
	...
	private void addOrChangeUnittypeImpl(Unittype unittypeXAPS, AddOrChangeUnittypeRequest gur) throws SQLException, RemoteException {
		...
		if (parameterList != null && parameterList.getParameterArray() != null) {
			for (com.github.freeacs.ws.xml.Parameter param : parameterList.getParameterArray().getItem()) {
				...
				if (param.getFlags() == null || param.getFlags().getValue().equals("AC")) {
					if (utp == null)
						acUtpList.add(new UnittypeParameter(unittypeXAPS, param.getName(), new UnittypeParameterFlag(param.getValue().getValue())));
					else {
		...
		try {
			...
			if (unittypes.getByName(gur.getUnittype().getName()) == null) { // make new unittype
				if (isAdmin) {// allow if login is admin
					...
					unittypeXAPS = new Unittype(uWS.getName(), uWS.getVendor().getValue(), uWS.getDescription().getValue(), ProvisioningProtocol.toEnum(uWS.getProtocol().getValue()));
					addOrChangeUnittypeImpl(unittypeXAPS, gur);
				} else {
					throw ACSFactory.error(logger, "The unittype " + gur.getUnittype().getName() + " does not exist, your login does not have the permissions to create it.");
				}

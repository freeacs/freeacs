...
public class DeleteProfile {
  ...
  public DeleteProfileResponse deleteProfile(
    ...
    try {
      ...
      if (dur.getUnittype() == null) throw ACSFactory.error(logger, "No unittype is specified");
      Unittype unittype = acsWS.getUnittypeFromXAPS(dur.getUnittype().getName());
      if (dur.getProfile() == null) throw ACSFactory.error(logger, "No profile name is specified");
      ...
      if (rowsDeleted > 0) return getDeleteProfileResponse(true);
      else return getDeleteProfileResponse(false);
    } catch (Throwable t) {

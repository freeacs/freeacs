...
public class TestMenu {
  ...
  public static void addOrChangeTestHistory(Unittype unittype, TestHistory testHistory)
      throws SQLException {
  ...
  public static List<TestHistory> listTestHistory(Unittype unittype, TestHistory filter)
      throws SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
    return testDB.getHistory(filter);
  }

  public static int importDirectory(Unittype unittype, String directory)
      throws IOException, SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
    TestCaseFileHandler tcfh = new TestCaseFileHandler(unittype);
    List<TestCase> testCaseList = tcfh.parseDirectory(directory);
    for (TestCase tc : testCaseList) testDB.addOrChangeTestCase(tc);
    return testCaseList.size();
  }

  public static TestCase importFile(Unittype unittype, String filename)
      throws IOException, SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
    TestCaseFileHandler tcfh = new TestCaseFileHandler(unittype);
  ...
  public static int exportTestCase(Unittype unittype, String directory, int testCaseId)
      throws IOException, SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
  ...
  public static int exportTestCase(
      Unittype unittype,
      String directory,
    ...
    Collection<TestCase> testCases =
        testDB.getCompleteTestCases(unittype, method, paramFilter, tagFilter);
    TestCaseFileHandler tcfh = new TestCaseFileHandler(unittype);
    for (TestCase tc : testCases) tcfh.toFile(directory, tc);
  ...
  public static List<TestCase> listTestCases(
      Unittype unittype, TestCaseMethod method, String paramFilter, String tagFilter)
      throws SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
    return testDB.getUncompleteTestCases(unittype, method, paramFilter, tagFilter);
  }

  public static Map<String, Integer> listTestCaseTags(
      Unittype unittype, TestCaseMethod method, String paramFilter, String tagFilter)
      throws SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
  ...
  public static int deleteTestCases(
      Unittype unittype,
      TestCaseMethod method,
      String paramFilter,
      String tagFilter,
    ...
    List<TestCase> testCases =
        testDB.getUncompleteTestCases(unittype, method, paramFilter, tagFilter);
    int count = 0;
    for (TestCase tc : testCases) {
      count++;
      testDB.deleteTestCase(tc);
      if (count % 100 == 0) session.println("Deleted " + count + " Test Cases");
    }
    return testCases.size();
  }

  public static int deleteDuplicateTestCases(Unittype unittype) throws SQLException {
    return deleteDuplicateTestCasesImpl(unittype, TestCaseMethod.VALUE)
        + deleteDuplicateTestCasesImpl(unittype, TestCaseMethod.ATTRIBUTE);
  }

  private static int deleteDuplicateTestCasesImpl(Unittype unittype, TestCaseMethod method)
      throws SQLException {
    TestDB testDB = new TestDB(unittype.getAcs());
    ...
    for (TestCase tc1 : testCases) {
      for (TestCase tc2 : testCases) {
        if (tc1.equals(tc2) && tc1.getId().intValue() != tc2.getId()) {
          if (tc1.getId().intValue() > tc2.getId()) deleteMap.put(tc1.getId(), tc1);
          else deleteMap.put(tc2.getId(), tc2);
        }
      }
    }
  ...
  public void showtc(String[] args, OutputHandler oh) throws SQLException {
    ...
    if (id == null) throw new IllegalArgumentException("The id was not a number");
    TestDB testDB = new TestDB(session.getAcs());
    TestCase tc = testDB.getTestCase(context.getUnittype(), id);
    if (tc == null) throw new IllegalArgumentException("No Test Case was found");
  ...
  public void exporttcdir(String args[]) throws SQLException, IOException {
    ...
    if (args.length > 2) {
      if (!args[2].equals("NULL")) method = TestCaseMethod.valueOf(args[2]);
    }
    if (args.length > 3) paramFilter = Util.autoboxString(args[3]);
    if (args.length > 4) tagFilter = Util.autoboxString(args[4]);

    int exported =
        TestMenu.exportTestCase(context.getUnittype(), args[1], method, paramFilter, tagFilter);
  ...
  public void exporttcfile(String[] args) throws IOException, SQLException {
    ...
    if (exported == 0) throw new IllegalArgumentException("The test case Id was not found");
    else session.println("Exported test case " + id);
  }
  ...
  public void generatetc(String args[]) throws Exception {
    ...
    for (TestCase tc : generatedTc) {
      count++;
      testDB.addOrChangeTestCase(tc);
      if (count % 100 == 0) session.println("Generated " + count + " Test Cases");
    }
  ...
  public void listtctags(String args[], OutputHandler oh) throws SQLException {
    ...
    String tagFilter = null;
    if (args.length > 1) {
      if (!args[1].equals("NULL")) method = TestCaseMethod.valueOf(args[1]);
    }
    if (args.length > 2) paramFilter = Util.autoboxString(args[2]);
    if (args.length > 3) tagFilter = Util.autoboxString(args[3]);
    Map<String, Integer> tagMap =
        TestMenu.listTestCaseTags(context.getUnittype(), method, paramFilter, tagFilter);
    Listing listing = oh.getListing();
    listing.setHeading("Tag", "TestCases");
    for (Entry<String, Integer> entry : tagMap.entrySet()) {
      if (entry.getValue() > 1) listing.addLine(entry.getKey(), "" + entry.getValue());
    }
  }
  ...
  public void listtc(String args[], OutputHandler oh) throws SQLException {
    ...
    for (TestCase tc : testCaseList) {
      ...
      if (tc.getExpectError() == null) line.addValue("NULL");
      else line.addValue(tc.getExpectError() ? "true" : "false");
  ...
  public void deltc(String args[]) throws SQLException {
    ...
    if (args.length > 3) tagFilter = Util.autoboxString(args[3]);
    int deleted =
        TestMenu.deleteTestCases(context.getUnittype(), method, paramFilter, tagFilter, session);
  ...
  public void deltesthistory(String args[]) throws SQLException {
    Date startTms = null;
    String unitId = null;
    if (context.getUnit() != null) unitId = context.getUnit().getId();
    if (args.length > 1) startTms = Util.autoboxDate(args[1]);
    TestHistory th = new TestHistory(context.getUnittype(), startTms, unitId, null, null);
    if (args.length > 2) th.setEndTimestamp(Util.autoboxDate(args[2]));
  ...
  public void listTestHistory(String[] args, OutputHandler oh) throws SQLException {
    ...
    if (context.getUnit() != null) filter.setUnitId(context.getUnit().getId());
    Listing listing = oh.getListing();
    Line headingLine =
        new Line("Start", "End", "TestCase", "Unitid", "Failed", "ExpectError", "Result");
    ...
    for (TestHistory th : testHistoryList) {
      ...
      if (th.getResult()
          != null) { // splits the result over several lines, to avoid very long lines
        String[] resultArray = th.getResult().split("\n");
        for (int i = 0; i < resultArray.length; i++) {
          if (i > 0) line = new Line("", "", "", "", "", "");
          line.addValue(resultArray[i]);
          listing.addLine(line);
        }
      } else listing.addLine(line);
    }
  }
  ...
  public void testModeChange(boolean enable) throws SQLException {
    if (enable) {
      session
          .getAcsUnit()
          .addOrChangeUnitParameter(context.getUnit(), SystemParameters.TEST_ENABLE, "1");
      session.println(
          "Test is enabled - you may initiate connection from device by sending a 'kick' command");
    } else {
      session
          .getAcsUnit()
          .addOrChangeUnitParameter(context.getUnit(), SystemParameters.TEST_ENABLE, "0");
      // will be read by MessageListenerTask in TR-069 Server - the change will be picked up by the
      // TR-069 server within max 11 seconds (average 5-6 sec)
      session
          .getDbi()
          .publishMessage(
              "N/A",
              Message.MTYPE_PUB_TR069_TEST_END,
              Message.OTYPE_UNIT,
              context.getUnit().getId(),

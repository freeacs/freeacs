...
public class StringUtil {
	...
	/**
	 * Replaces the old split function, since the old function did
	 * not handle strings which had quotes in the middle of a string.
	 * This was a problem when using the special root-char :. For
	 * example this string would not be properly split:
	 * 	call script.xss :"Testperf" TestperfP;
	 * @param s
	 * @return
	 */
	...
	/**
	 * A command string must be splitted on |, because it signals that the output of the first
	 * command should go into the next command. However, if the | is within quotes, it must be
	 * ignored an no splitting should happen.
	 * @param s
	 * @return
	 */

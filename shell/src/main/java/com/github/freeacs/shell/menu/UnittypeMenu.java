...
public class UnittypeMenu {
  ...
  private void listprofiles(String[] args, OutputHandler oh) throws Exception {
    ...
    for (int i = 0; i < profiles.length; i++) {
      if (!Validation.matches(args.length > 1 ? args[1] : null, profiles[i].getName())) continue;
      listing.addLine(
          profiles[i].getName(),
          "" + profiles[i].getProfileParameters().getProfileParameters().length);
    }
  }
  ...
  private void listparams(String[] args, OutputHandler oh) throws Exception {
    ...
    for (int i = 0; i < unittypeParameters.length; i++) {
      if (flagArg != null) {
        ...
        if (flagArg.contains("R") && flagArg.contains("W") && !utpFlag.isReadWrite()) continue;
        if (flagArg.contains("R") && !flagArg.contains("W") && !utpFlag.isReadOnly()) continue;
        if (flagArg.contains("C") && !utpFlag.isConfidential()) continue;
        if (flagArg.contains("A") && !utpFlag.isAlwaysRead()) continue;
        if (flagArg.contains("B") && !utpFlag.isBootRequired()) continue;
        if (flagArg.contains("D") && !utpFlag.isDisplayable()) continue;
        if (flagArg.contains("S") && !utpFlag.isSearchable()) continue;
        if (flagArg.contains("X") && !utpFlag.isSystem()) continue;
      }
      if (!Validation.matches(matchArg, unittypeParameters[i].getName())) continue;
  ...
  private void listexecutions(String[] args, OutputHandler oh) throws Exception {
    ...
    for (ScriptExecution se : executionList) {
      if (!Validation.matches(
          ...
          se.getArguments(),
          se.getErrorMessage(),
          se.getRequestId())) continue;
      String exitStatus = "NULL";
      if (se.getExitStatus() != null) exitStatus = se.getExitStatus() ? "ERROR" : "OK";
      listing.addLine(
          se.getScriptFile().getName(),
          se.getArguments(),
          "" + se.getRequestTms(),
  ...
  private void exportfile(String[] args) throws Exception {
    ...
    if (f == null) throw new IllegalArgumentException("The file does not exist");
  ...
  private void importfile(String[] args) throws Exception {
    ...
    if (fusion2012) { // This logic is created to read 2012R1 files output and import into into
      ...
      if (!dateStr.equals("NULL")) date = dateFormat.parse(dateStr);
    } else {
      ver = args[3];
  ...
  private void delfile(String[] args) throws Exception {
    ...
    if (f == null) throw new IllegalArgumentException("The file " + args[1] + " does not exist");
  ...
  private void setsyslogevent(String[] args) throws Exception {
    ...
    if (eventId < 1000) expression = null;
  ...
  private void listsyslogevents(String[] args, OutputHandler oh) throws Exception {
    ...
    for (int i = 0; i < syslogEvents.length; i++) {
      ...
      if (se.getGroup() != null) line.addValue(se.getGroup().getName());
      else line.addValue("NULL");
      if (se.getExpression() != null) line.addValue(se.getExpression().toString());
      else line.addValue("NULL");
      line.addValue(se.getStorePolicy().toString());
      if (se.getScript() != null) line.addValue(se.getScript().getName());
      else line.addValue("NULL");
      if (se.getDeleteLimit() != null) line.addValue("" + se.getDeleteLimit());
      else line.addValue("NULL");
  ...
  // <description> <software-version> <unconfirmed-timeout> <move-to-profile>|NULL <stop-rules>|NULL
  ...
  private void setjob(String[] args) throws Exception {
    ...
    Job dependency = null;
    if (!parent.equals("NULL")) dependency = context.getUnittype().getJobs().getByName(parent);
    ...
    if (!args[9].equals("NULL")) stopRules = args[9];
    ...
    if (args.length > 11) {
      try {
        if (!args[10].equals("NULL")) {
          ...
          if (!args[11].equals("NULL")) repeatInterval = Integer.parseInt(args[11]);
          else repeatInterval = 86400;
        }
      } catch (NumberFormatException nfe) {
  ...
  private void listjobs(String[] args, OutputHandler oh) throws Exception {
    ...
    for (Job job : jobs) {
      if (job.getGroup().getUnittype().getId().intValue()
          == context.getUnittype().getId().intValue()) filteredJobs.add(job);
    }
    if (args.length > 2) {
      jobname = args[1];
    ...
    for (int i = 0; i < filteredJobs.size(); i++) {
      ...
      if (jobname != null
          && !Validation.matches(
              ...
              job.getDescription(),
              job.getFile().getVersion())) continue;
      Line line = new Line();
      ...
      if (job.getDependency() == null) line.addValue("NULL");
      else line.addValue(job.getDependency().getName());
      line.addValue(job.getDescription());
      if (job.getFile() == null) line.addValue("NULL");
      else line.addValue(job.getFile().getVersion());
      line.addValue("" + job.getUnconfirmedTimeout());
      //			if (job.getMoveToProfile() == null)
      //				line.addValue("NULL");
      //			else
      //				line.addValue(job.getMoveToProfile().getName());
      if (job.getStopRules() == null || job.getStopRules().size() == 0) line.addValue("NULL");
      else line.addValue(job.getStopRulesSerialized());
      if (job.getRepeatCount() == null) line.addValue("NULL");
      else line.addValue("" + job.getRepeatCount());
      if (job.getRepeatInterval() == null) line.addValue("NULL");
      else line.addValue("" + job.getRepeatInterval());
      listing.addLine(line);
    }
  }
  ...
  private void listtriggers(String[] args, OutputHandler oh) throws Exception {
    ...
    for (int i = 0; i < triggers.length; i++) {
      ...
      if (trigger.getScript() != null) scriptName = trigger.getScript().getName();
      ...
      if (trigger.getParent() != null) parentTriggerName = trigger.getParent().getName();
      if (!Validation.matches(
          searchStr,
          trigger.getName(),
      ...
      if (trigger.getToList() == null) line.addValue("NULL");
      else line.addValue("" + trigger.getToList());
  ...
  private void listgroups(String[] args, OutputHandler oh) throws Exception {
    ...
    if (args.length > 2) {
      searchStr = args[1];
      if (args[2].equalsIgnoreCase("PARENT-FIRST")) Arrays.sort(groups, new GroupComparator(true));
      else if (args[2].equalsIgnoreCase("PARENT-LAST"))
        Arrays.sort(groups, new GroupComparator(false));
    } else if (args.length > 1) {
      if (args[1].equalsIgnoreCase("PARENT-FIRST")) Arrays.sort(groups, new GroupComparator(true));
      else if (args[1].equalsIgnoreCase("PARENT-LAST"))
        Arrays.sort(groups, new GroupComparator(false));
      else searchStr = args[1];
    }
    ...
    for (int i = 0; i < groups.length; i++) {
      ...
      Line line = new Line();
      if (searchStr != null
          && !Validation.matches(searchStr, group.getName(), group.getDescription())) continue;
      line.addValue(group.getName());
      if (group.getParent() == null) line.addValue("NULL");
      else line.addValue(group.getParent().getName());
      line.addValue(group.getDescription());
      if (group.getTopParent().getProfile() == null) line.addValue("NULL");
      else line.addValue(group.getTopParent().getProfile().getName());
      ...
      if (group.getCount() != null) line.addValue("" + group.getCount());
      else line.addValue("Not counted yet");
      listing.addLine(line);
    }
  }
  ...
  private void settrigger(String[] args) throws Exception {
    ...
    if (args[11].equals("NULL")) thisTrigger.setSyslogEvent(null);
    else
  ...
  private void setexecution(String[] args) throws Exception {
    ...
    if (spacePos == -1) scriptFileStr = args1Str;
    else {
  ...
  private void setgroup(String[] args) throws Exception {
    ...
    if (args[2].equals("NULL")) thisGroup.setParent(null);
    else {
    ...
    if (args[4].equals("NULL")) thisGroup.setProfile(null);
    else {
    ...
    //		} else {
    //			UnittypeParameter timeParameter =
    // context.getUnittype().getUnittypeParameters().getByName(args[5]);
    //			if (timeParameter == null)
    //				throw new IllegalArgumentException("[" + session.getCounter() + "] Time parameter
    // specified is not a unittype parameter");
    //			else
    ...
    //			} catch (NumberFormatException nfe) {
    //				throw new IllegalArgumentException("[" + session.getCounter() + "] Time offset is not a
    // number");
    //			}
    //		}
  ...
  private void systemparameterscleanup(String[] args) throws Exception {
    ...
    for (UnittypeParameter utp : utps.getUnittypeParameters()) {
      if (utp.getName().startsWith("System.X_FREEACS-COM.")) {
        if (SystemParameters.commonParameters.get(utp.getName()) != null) continue;
        if (SystemParameters.stagingParameters.get(utp.getName()) != null) continue;
        deleteList.add(utp);
      }
    }

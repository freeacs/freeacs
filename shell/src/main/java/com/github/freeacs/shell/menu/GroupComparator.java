...
public class GroupComparator implements Comparator<Group> {
  ...
  public int compare(Group g1, Group g2) {
    if (parentFirst) return -compareImpl(g1, g2);
    else return compareImpl(g1, g2);
  }
  ...
  private int compareImpl(Group g1, Group g2) {
    ...
    Group tmp1 = g1;
    List<Integer> groupIdList1 = new ArrayList<Integer>();
    while (tmp1.getParent() != null) {
      tmp1 = tmp1.getParent();
      groupIdList1.add(tmp1.getId());
    }
    Group tmp2 = g2;
    List<Integer> groupIdList2 = new ArrayList<Integer>();
    ...
    if (result > 0) {
      ...
      //			System.out.println("TOP  UP   : " + g1.getName() + " compared to " + g2.getName());
      return result;
    }
    ...
    if (matchId != null) {
      ...
      /* No match, the Groups are totally unrelated. The order assigned is therefore rather arbitrary */
    ...
    /* The groups are list of Groups to the top are the same, possibly sibling Groups. Regular
     * string compare on the name decides the matter.
     */
    result = g1.getName().compareTo(g2.getName());
    if (result >= 0) {
      //			System.out.println("ALFA DOWN : " + g1.getName() + " compared to " + g2.getName());
      return result;
    } else {
      //			System.out.println("ALFA UP   : " + g1.getName() + " compared to " + g2.getName());
      return result;
    }
  }
}

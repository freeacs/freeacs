...
public class ProfileMenu {
	...
	private void listparams(String[] inputArr, OutputHandler oh) throws Exception {
		ProfileParameter[] profileParameters = context.getProfile().getProfileParameters().getProfileParameters();
		Listing listing = oh.getListing();
		listing.setHeading("Unittype parameter name", "Value");
		for (int i = 0; i < profileParameters.length; i++) {
			if (!Validation.matches(inputArr.length > 1 ? inputArr[1] : null, profileParameters[i].getUnittypeParameter().getName(), profileParameters[i].getValue()))
				continue;
			listing.addLine(profileParameters[i].getUnittypeParameter().getName(), profileParameters[i].getValue());
		}
	}

	private void setparam(String[] args) throws Exception {
		Validation.numberOfArgs(args, 3);
		ProfileParameter profileParameter = context.getProfile().getProfileParameters().getByName(args[1]);
		String action = "";
		UnittypeParameter up = context.getUnittype().getUnittypeParameters().getByName(args[1]);
		if (up == null) {
			throw new IllegalArgumentException("The profile parameter " + args[1] + " is not a unittype parameter in unittype " + context.getUnittype().getName());
		} else {

...
public class Arguments {
  ...
  public Arguments(String[] args) {
    ...
    for (String arg : args) {
      if (arg.startsWith("-")) {
        if (flag != null) argMap.put(flag, FLAG_EXISTS);
        flag = arg.substring(1);
      } else {
        if (flag == null) throw new IllegalArgumentException("No - prefixing flag option");
        else {
          argMap.put(flag, arg);
          flag = null;
        }
  ...
  public String getArgument(String flag) {
    ...
    if (value == null && argMap.containsKey(flag)) return FLAG_EXISTS;
    else return value;
  }
}

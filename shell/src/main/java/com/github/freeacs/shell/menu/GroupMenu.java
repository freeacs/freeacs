...
public class GroupMenu {
	...
	private void setparam(String[] inputArr) throws Exception {
		...
		if (params != null) {
			...
			for (Parameter param : params) {
				...
				if (param.getGroupParameterId() == null || groupParams.getById(param.getGroupParameterId()) == null) {
					gp = new GroupParameter(param, group);
				} else {
					gp = groupParams.getByName(param.getUnittypeParameter().getName() + "#" + param.getGroupParameterId());
	...
	private void listparams(String[] args, OutputHandler oh, boolean export) throws Exception {
		...
		for (GroupParameter gp : gpArr) {
			...
			if (!Validation.matches(args.length > 1 ? args[1] : null, p.getUnittypeParameter().getName(), p.getValue(), p.getOp().getOperatorLiteral(), p.getType().getType()))
				continue;
			Line line = new Line();
			if (export)
	...
	private void listdetails(String[] args, OutputHandler oh) throws Exception {
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			for (int j = 0; j < groups.get(i).getGroupParameters().getGroupParameters().length; j++) {
				...
				oh.print(String.format("%-30s " + gp.getParameter().getOp().getOperatorLiteral() + " ", gp.getParameter().getUnittypeParameter().getName()));
				if (gp.getParameter().getValue() == null || gp.getParameter().valueWasNull())
					oh.print("NULL\n");
				else

...
public class Validation {
	public static void numberOfArgs(String[] args, int argsExpected) {
		if (args.length < argsExpected) {
			if (args.length <= 1)
				throw new IllegalArgumentException("No arguments supplied, expected at least " + (argsExpected - 1));
			if (args.length > 1) {
				String supplied = "";
				...
				throw new IllegalArgumentException("Too few argument (expected  at least " + (argsExpected - 1) + ") (arguments supplied: " + supplied + ")");
			}
		}
	}
	...
	public static boolean filter(String[] args, String str) {
		...
		if (patternStr != null) {
			...
			try {
				...
				throw new IllegalArgumentException("The filter argument is not allowed: " + pse.getMessage() + "(reason may be not expanded variable)");
			}
	...
	/**
	 * The method is a slightly modified version of String.matches(), since it also
	 * handles a ! at the start of the regex to negate the search. Also handles
	 * a non-existing regex  and non-existing strToMatch
	 *
	...
	/**
	 * The method will match against all the string. If none matches, this

		...
		while ((unitFileLine = unitFileBr.readLine()) != null) {
			...
			for (Entry<String, UnitParameter> entry : unitParameters.entrySet()) {
				if (entry.getKey().endsWith("ConnectTms"))
					continue;
				if (entry.getKey().endsWith("DesiredSoftwareVersion"))
					continue;
				if (convertParamMap.get(entry.getKey()) != null) {
					...
					for (String newParam : newParams) {						
						UnittypeParameter newUtp = newUnittype.getUnittypeParameters().getByName(newParam.trim());
						if (newUtp == null)
							System.out.println("Couldn't find " + convertParamMap.get(entry.getKey()) + " in new Unittype");
						newUnitParams.add(new UnitParameter(newUtp, newUnit.getId(), entry.getValue().getValue(), profile));
					}
				}
				//				else {
				//					String newParamName = entry.getKey().replaceFirst("Device.", "InternetGatewayDevice.");
				//					UnittypeParameter newUtp = newUnittype.getUnittypeParameters().getByName(newParamName);
				//					if (newUtp == null)
				//						System.out.println("Couldn't find " + entry.getKey() + " in new Unittype - the parameter is removed entirely");
				//					else
				//						newUnitParams.add(new UnitParameter(newUtp, newUnit.getId(), entry.getValue().getValue(), profile));
				//				}
			}

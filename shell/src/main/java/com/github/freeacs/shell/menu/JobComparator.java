...
public class JobComparator implements Comparator<Job> {
  ...
  public int compare(Job j1, Job j2) {
    if (dependencyFirst) return -compareImpl(j1, j2);
    else return compareImpl(j1, j2);
  }
  ...
  private int compareImpl(Job j1, Job j2) {
    ...
    Job tmp1 = j1;
    List<Integer> jobIdList1 = new ArrayList<Integer>();
    ...
    if (result > 0) {
      ...
      //			System.out.println("TOP  UP   : " + j1.getName() + " compared to " + j2.getName());
      return result;
    }
    ...
    if (matchId != null) {
      ...
      /* No match, the jobs are totally unrelated. The order assigned is therefore rather arbitrary */
      if (jobIdList1.size() == 0) {
    ...
    /* The groups are list of jobs to the top are the same, possibly sibling jobs. Regular
     * string compare on the name decides the matter.
     */
    result = j1.getName().compareTo(j2.getName());
    if (result >= 0) {
      //			System.out.println("ALFA DOWN : " + j1.getName() + " compared to " + j2.getName());
      return result;
    } else {
      //			System.out.println("ALFA UP   : " + j1.getName() + " compared to " + j2.getName());
      return result;
    }
  }
}

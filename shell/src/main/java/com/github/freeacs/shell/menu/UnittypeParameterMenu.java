...
public class UnittypeParameterMenu {
	...
	private void generateenum(String[] inputArr, OutputHandler oh) throws Exception {
		...
		if (dmp.getEnumeration() != null && dmp.getEnumeration().size() > 0 && dmp.getEnumeration().get(0).getValue() != null) {
	...
	private void setvalues(String[] args) throws Exception {
		...
		if (!args[1].equals(UnittypeParameterValues.ENUM) && !args[1].equals(UnittypeParameterValues.REGEXP))
			throw new IllegalArgumentException("The first argument must be either " + UnittypeParameterValues.ENUM + " or " + UnittypeParameterValues.REGEXP);

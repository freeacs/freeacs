...
import com.owera.xaps.shell.sync.Sync;
...
public class RootMenu {
	...
	public void executescript() throws Exception {
		if (session.getBatchStorage().getAddChangeUnittypeParameters().size() > 0) {
			try {
				...
				ut.getUnittypeParameters().addOrChangeUnittypeParameters(session.getBatchStorage().getAddChangeUnittypeParameters(), session.getXaps());
		...
		if (session.getContext().getProfile() != null) {
			...
			XAPSUnit xapsU = session.getXapsUnit();
			try {
		...
		if (session.getBatchStorage().getDeleteUnittypeParameters().size() > 0) {
			try {
				...
				ut.getUnittypeParameters().deleteUnittypeParameters(session.getBatchStorage().getDeleteUnittypeParameters(), session.getXaps());
	...
	private void listperms(String[] args, OutputHandler oh) throws Exception {
		...
		for (User user : users.getUsers(session.getVerifiedFusionUser())) {
			...
			for (Permission p : permissions) {
				Unittype unittype = session.getXaps().getUnittype(p.getUnittypeId());
	...
	private void listcertificates(String[] args, OutputHandler oh) throws IOException {
		Certificates certificates = session.getXaps().getCertificates();
		Listing listing = oh.getListing();
	...
	private void listunittypes(String[] args, OutputHandler oh) throws IOException {
		Unittype[] unittypes = session.getXaps().getUnittypes().getUnittypes();
		Listing listing = oh.getListing();
	...
	private void delperm(String[] args) throws Exception {
		...
		Unittype unittype = session.getXaps().getUnittype(unittypeName);
		if (unittype == null)
	...
	private void delcertificate(String[] args) throws Exception {
		...
		if (cert == null)
			...
			certificates.deleteCertificate(cert, session.getXaps());
	...
	private void setcertificate(String[] args) throws Exception {
		...
		certificates.addOrChangeCertificate(cert, session.getXaps());
	...
	private void setunittype(String[] args, OutputHandler oh) throws Exception {
		...
		if (args.length >= 5) {
			Unittype unittype = session.getXaps().getUnittype(args[1]);
			String action = "";
			if (unittype == null) {
	...
	private void delunittype(String[] args) throws Exception {
		...
		if (unittype == null)
			...
			session.getXaps().getUnittypes().deleteUnittype(unittype, session.getXaps(), true);
	...
	private Map<String, Unit> getUnits(String[] args) throws Exception {
		...
		if (args.length > 1)
			units = session.getXapsUnit().getUnits("%" + args[1] + "%", null, null, null);
		else
			units = session.getXapsUnit().getUnits((String) null, null, null, null);
		return units;
	}

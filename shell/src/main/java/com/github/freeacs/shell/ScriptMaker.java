...
public class ScriptMaker {
  ...
  public static List<String> getDeleteScript(int i, String[] args) {
    ...
    if (unittypeName.equals("ALL")) script.add("listunittypes > unittypes_sd.txt");
    else script.add("listunittypes \"^" + unittypeName + "$\" > unittypes_sd.txt");
    ...
    if (unittypeName.equals("ALL")) script.add("listperms > perms_sd.txt");
    else script.add("listperms \"^" + unittypeName + "$\" > perms_sd.txt");
    script.add("delperm -u < perms_sd.txt");
    if (ACSVersionCheck.heartbeatSupported) script.add("delheartbeat -u < heartbeats_sd.txt");
    if (ACSVersionCheck.triggerSupported) script.add("deltrigger -u < triggers_sd.txt");
  ...
  private static List<String> getExportScript(String unittypeName, String path) {
    ...
    if (unittypeName.equals("ALL")) scriptLines.add("listperms -c > " + path + "perms.txt");
    else scriptLines.add("listperms -c \"^" + unittypeName + "$\" > " + path + "perms.txt");
  ...
  private static List<String> getImportScript(String path) {
    ...
    if (FileUtil.exists(path + "users.txt")) scriptLines.add("setuser < " + path + "users.txt");
    if (FileUtil.exists(path + "perms.txt")) scriptLines.add("setperm < " + path + "perms.txt");
    ...
    if (fileLines.size() > 0
        && StringUtil.split(fileLines.get(0)).length == 6) // able to read 2012R1-files.txt
  ...
  public static List<String> getMigrateScript(int i, String[] args) {
    ...
    File dir = null;
    if (folder != null) folder = folder + File.separator + unittypeName;
    else folder = unittypeName;
  ...
  private static String getUnittypeFromArg(int i, String[] args) {
    ...
    if (args.length >= i + 2) unittypeName = args[i + 1];

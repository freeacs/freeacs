...
public class ScriptMaker {
	...
	public static List<String> getDeleteScript(int i, String[] args) {
		...
		if (unittypeName.equals("ALL"))
			...
			script.add("echo WARNING: The unittype \"" + unittypeName + "\" will be deleted, press CTRL-C to abort");
	...
	private static List<String> getExportScript(String unittypeName, String path) {
		...
		scriptLines.add("pr:${1}/ listparams -c -u < " + path + "profiles.txt > " + path + "profileparams.txt");
		scriptLines.add("pr:${1}/ listunits -c -u < " + path + "profiles.txt > " + path + "units.txt");
		scriptLines.add("un:${1}/ listunitparams -c -u < " + path + "units.txt > " + path + "unitparams.txt");
		scriptLines.add("/ut:${1}/ listfiles -c < " + path + "unittypes.txt > " + path + "files.txt");
		scriptLines.add("/ut:${1}/ listgroups -c parent-first < " + path + "unittypes.txt > " + path + "groups.txt");
		scriptLines.add("/ut:${1}/ listjobs -c dep-first < " + path + "unittypes.txt > " + path + "jobs.txt");
	...
	private static List<String> getImportScript(String path) {
		...
		if (fileLines.size() > 0 && StringUtil.split(fileLines.get(0)).length == 6) // able to read 2012R1-files.txt
			scriptLines.add("importfile -u " + path + "${1} ${2} ${3} ${4} ${5} < " + path + "files.txt");
		else
			scriptLines.add("importfile -u " + path + "${1} ${2} ${3} ${4} ${5} ${6} ${7} < " + path + "files.txt");
		...
		if (XAPSVersionCheck.heartbeatSupported && (new File(path + "heartbeats.txt")).exists())
		...
		if (XAPSVersionCheck.triggerSupported && (new File(path + "triggers.txt")).exists())
		...
		for (int j = 0; j < args.length; j++) {
			if (args[j].equals("-dir")) {
				try {
					...
					//					XAPSShell.println("No directory specified, using working directory to place the " + unittypeName + "directory");
				}
			}
		}
		if (folder != null) {
			...
			if (f1.exists() && !f1.isDirectory()) {
				throw new IllegalArgumentException("Cannot import/export since " + unittypeName + " is not a directory");
			} else {
				f1.mkdir();
			}
		}
	...
	private static String getUnittypeFromArg(int i, String[] args) {
		...
		if (unittypeName == null)
			throw new IllegalArgumentException(args[i] + " must be followed by an argument specifying unittype name");

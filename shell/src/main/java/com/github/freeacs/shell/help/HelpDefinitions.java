...
public class HelpDefinitions {
	...
	private String getPatternComment(String argName) {
		String argCom = "Optional. Any string which will be used to match the list of " + argName + "s. ";
		...
		argCom += "you need to consult internet resources. Special feature: Prefix with '!' to negate the search.";
		return argCom;
	}
	...
	private HelpGroup jobHelpBuilder() {
		...
		help.addArgument("[DEFAULT|<unitid>]", "If set to DEFAULT the command will list all job parameters that are common for all units run by this job. "
				...
				+ "to the unit are the set of DEFAULT job parameters + the unit specific job parameters. If argument is skipped, all job parameters will be " + "listed.");
		help.addExamples("listparams", "listparams DEFAULT", "listparams 123456-Router-123456789AB");
		hg.addHelp(help);

		help = new Help("listfailedunits");
		help.addComment("Will list all units that has had a failure of sorts when running the job. It might be that the unit in the end has managed to execute the job.");
		help.addOption(getAllInfoHelpOption());
		help.addOption(getUseContextHelpOption());
		help.addOption(getListContextHelpOption());
		help.addOption(getOrderHelpOption());
		...
		help.addComment("Will delete all records of units that failed when running the job. This command is mostly used for export/delete of the entire unittype, "
		...
		help.addComment("Finishes the job, it cannot be started after this. You cannot finish a job before you have stopped it.");
		help.addOption(getUseContextHelpOption());
		hg.addHelp(help);
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("DEFAULT|<unitid>", "Set to DEFAULT to create a job parameter that will apply to all units executing this job. Set to <unitid> to create a job parameter specific to a unit.");
		help.addArgument("<unittype-parameter-name>", "Adding: A valid unittype parameter. Changing: A valid job parameter.");
		help.addArgument("<value>", "Any string. Max 255 characters. The value will be set as a unit parameter on the unit executing the job.");
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("DEFAULT|<unitid>", "Set to DEFAULT to delete a job parameter that will apply to all units executing this job. Set to <unitid> to delete a job parameter specific to a unit.");
		...
		help.addComment("Refresh status about a job. Otherwise counters seen in 'listjobdetails' are not updated in this session.");
		help.addOption(getUseContextHelpOption());
		hg.addHelp(help);
	...
	private HelpOption getUseContextHelpOption() {
		return new HelpOption("-u", "Will use the context available from another command (piping) or input-file. The context will be changed before the command is run.");
	}

	private HelpOption getAllInfoHelpOption() {
		return new HelpOption("-a", "Will list all available information about this object to the output. This is useful for export of data.");
	}

	private HelpOption getListContextHelpOption() {
		return new HelpOption("-c", "Will list the current context to the output. This is useful when using this output as input to another command using the -u option.");
	}

	private HelpOption getOrderHelpOption() {
		return new HelpOption("-o", "Will order the listing of the output. Syntax of the option argument is -o(<columnnumber><a|n><a|d>)+ An example might be "
	...
	private HelpGroup groupHelpBuilder() {
		...
		help.addComment("Add/change a group parameter. A group parameter is acutally a search criteria, hence the equal/negation character.");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<unittype-parameter-name>", "Adding: A valid unittype parameter name. Changing: A valid group parameter with the #<number> at the end.");
		help.addArgument("<operator>", "Valid operators are EQ (equals), NE (not equals), LT (less), LE (less or equal), " + "GE (greater or equal), GT (greater).  ");
		help.addArgument("<value>", "Any string. Max 255 characters. Long values will take more time to process in the event of a group search, so try to avoid if possible. "
		...
		help.addArgument("[<dataype>]", "Optional. Default datatype is VARCHAR (which is the same as 'text' or 'string'). You can also "
		...
		help.addComment("Delete a group parameter. This will change the search criteria for this group, and all child groups of this group.");
		help.addOption(getUseContextHelpOption());
		...
		help.addComment("Delete all group parameters. This will change the search criteria for this group, and all child groups of this group.");
		help.addOption(getUseContextHelpOption());
		hg.addHelp(help);
	...
	private Help getListTestHistoryHelp() {
		...
		help.addComment("List the test history of this unit. Takes a number of optional filter parameters on the form 'filtername=value'.");
		help.addArgument("startTms=<timestamp>", "Optional. If specified, only shows tests that started after the specified date.");
		help.addArgument("endTms=<timestamp>", "Optional. If specified, only shows tests that finished before the specified date.");
		help.addArgument("result=<expression>", "Optional. If specified, only shows tests whose results matches the given expression." +
		" In the expression, '_' means 'exactly one wildcard' and '%' means 'zero or more wildcards'.");
		help.addArgument("expectError=<boolean>", "Optional. If specified, only shows tests that either expects errors or not.");
		help.addArgument("failed=<boolean>", "Optional. If specified, only shows tests that failed or not.");
		help.addExamples("listtesthistory", "listtesthistory startTms=3d1h",
	...
	private HelpGroup unittypeParameterHelpBuilder() {
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("enum|regexp", "The only supported 'mode' in Fusion Web is 'enum'. Later it might be possible to specify a regexp with which a parameter value must match.");
		help.addArgument("<value>+", "One or more values. If a value contains spaces enclose value with double quotes.");
		...
		help.addComment("Generates enumeration values based on TR098-1.4, TR104-1.1 and TR181-1.2.0. Be advised that the enumerations may " + "not be supported by the device.");
	...
	private HelpGroup unitHelpBuilder() {
		...
		help.addComment("List all parameters, both profile and unit parameter. Last column shows which are unit parameters (U) and which are profile parameters (P).");
		help.addOption(getAllInfoHelpOption());
		help.addOption(getUseContextHelpOption());
		help.addOption(getListContextHelpOption());
		help.addOption(getOrderHelpOption());
		help.addArgument("[[!]<parameter-name-pattern>]", getPatternComment("parameter-name"));
		help.addExamples("listallparams", "listallparams DeviceInfo", "listallparams ^Device.DeviceInfo.*");
		hg.addHelp(help);
		...
		help.addOption(getOrderHelpOption());
		help.addArgument("[[!]<parameter-name-pattern>]", getPatternComment("parameter-name"));
		help.addExamples("listunitparams", "listunitparams DeviceInfo", "listunitparams ^Device.DeviceInfo.*");
		hg.addHelp(help);
		...
		String argCom = "The device will connect ASAP and complete a provisioning (both reading/writing from/to the device). ";
		argCom += "This will only work if the ConnectionRequestURL or UDPConnectionRequestAddress is pointing to the correct address.";
		help.addComment("");
		help.addOption(getUseContextHelpOption());
		help.addArgument("async", "If this argument is used, the kick will be initiated and then the command will return. "
		...
		argCom = "The device will connect ASAP and all parameters from the CPE/device will be read and listed with the 'listallparameters' command. ";
		argCom += "This will only work if the ConnectionRequestURL or UDPConnectionRequestAddress is pointing to the correct address.";
		...
		argCom = "The device will return to REGULAR mode without issuing any kick to the device. The next time the device connects to the server, it will be a normal provisioning session";
		help.addComment(argCom);
		help.addExamples("regular");
		hg.addHelp(help);

		help = new Help("refresh");
		help.addComment("Refresh the unit parameter cache in shell. This may be necessary to run also if you've used the setparam command, since setparam do not"
		...
		help.addComment("If the optional arugment is omitted, it will generate Test Cases for the TR-069 test based on the Unit type parameters available and TR-181 (v1.9), TR-104 (v1.1) and TR-098 (v1.4)");
		help.addArgument("test-case-id", "Specify a \"master\" Test Case with many N SET parameters. The generate command will produce one TC for each SET parameter, from 1-N params");
		...
		help.addArgument("<method>|NULL", "Optional. Could be set to VALUE, ATTRIBUTE or FILE. Setting it to NULL will match all (methods)");
		help.addArgument("<parameter-filter>|NULL", "Optional if the previous argument is specified. Could be set to a part of a parameter name. Setting it to NULL will match all (parameters)");
		help.addArgument("<paratagmeter-filter>|NULL",
		...
		String comment = "List all tags for these test cases, and count the number of test case occurrences for each tag. ";
		comment += "The tags used are (according to the format of a test case) entirely freely chosen values - hence the meaning ";
		comment += "of a tag is up to the creator of a test case. However, using the generatetc command, a number of tags will be ";
		...
		comment += "\tLAYERx      - Testing all parameters on layer x in the unittype. Layer 0 is ALL parameters, Layer 1 could be all params within DeviceInfo object, etc\n";
		...
		help.addArgument("<method>|NULL", "Optional. Could be set to VALUE or ATTRIBUTE. Setting it to NULL will match all (methods)");
		help.addArgument("<parameter-filter>|NULL", "Optional if the previous argument is specified. Could be set to a part of a parameter name. Setting it to NULL will match all (parameters)");
		help.addArgument("<paratagmeter-filter>|NULL",
				"Optional if the previous argument is specified. Could be set to a series of strings enclosed in square brackets [ ] to match tags. Setting it to NULL will match all (tags)");
		...
		help = new Help("exporttcdir <directory> [<method>|NULL [<parameter-filter>|NULL [<tag-filter>|NULL]]]");
		...
		help = new Help("testsetup [steps=<steplist>] [reset=true|false] " +
				"[method=VALUE|ATTRIBUTE|FILE] [parameter-filter=<paramname>] [tag-filter=<taglist>]");
		comment = "Setup the TR-069 parameter test. We suggest to start with the first example, assuming you've run the " +
				"generatetc command. Then continue with the following examples in the list."; 
		help.addComment(comment);
		help.addArgument("<steps>", "A comma-separated list of the following allowed string: " +
		...
		help.addArgument("<reset>", "True or False, 0 or 1. Will initiate a Factory Reset " +
				"before the test starts if set to True/1. False/0 by default.");
		help.addArgument("<method>", "Can be either VALUE, ATTRIBUTE or FILE. If set to VALUE, " +
		...
		help.addArgument("<parameter-filter>", "A part of a parameter name which will narrow the set " +
				"of test-cases to be run in the test.");
		help.addArgument("<tag-filter>", "A set of tags, each enclosed is square brackets, which will " +
		...
		help.addExample("testsetup steps=SET,REBOOT,GET param-filter=WANConnection tag-filter=\"[READWRITE] [GENERATED]\"");
		
		hg.addHelp(help);

		help = new Help("enabletest");
		help.addComment("Enable TR-069 parameter test mode, which performs a series of tests based on test-cases added to the Unit Type. As long as the device is in test-mode, it will not "
		...
		help.addArgument("startTms", "Optional, can also be set to NULL (to delete from beginning). Otherwise specify date using syntax from syslog-command");
		help.addArgument("endTms", "Optional, can also be set to NULL (to delete until end). Otherwise specify date using syntax from syslog-command");
		help.addExamples("deltesthistory", "deltesthistory 3d 1h", "deltesthistory 20120701 20120705-1300");
		hg.addHelp(help);
	...
	private HelpGroup profileHelpBuilder() {
		...
		help.addOption(getListContextHelpOption());
		help.addOption(getOrderHelpOption());
		help.addArgument("[[!]<profile-parameter-name-pattern>]", getPatternComment("profile-parameter"));
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("<parameter-name>", "Adding: A valid unittype parameter name. Changing: A valid profile parameter name.");
		...
		help = new Help("listunits [(<search-value>) | (<unittype-parameter-name> <operator> <value> [<datatype>])*]");
		help.addComment("List units within this unittype.");
		help.addOption(getAllInfoHelpOption());
		help.addOption(getUseContextHelpOption());
		help.addOption(getListContextHelpOption());
		help.addOption(getOrderHelpOption());
		help.addArgument("[<search-value>]", "Optional. It will search among all parameter values and unit-ids");
		help.addArgument("<operator>", "Valid operators are EQ (equals), NE (not equals), LT (less), LE (less or equal), " + "GE (greater or equal), GT (greater).  ");
		help.addArgument("<value>", "Any string. Max 255 characters. Long values will take more time to process in the event of a group search, so try to avoid if possible. "
				+ "The rules for matching are the same as for the 'listunits' commands in the unittype/profile context. In other words, use '%' in the value string to match 0 "
				+ "or more arbitrary characters and use '_' in the value string to match 1 arbitrary character. Avoid % and _ at the start of the value. To search "
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("<unitid>", "A TR-069 unit id must adhere to the standard which is\n\t<OUI>[-<ProductClass>]<-SerialNumber>.\n"
		...
		help.addComment("Deletes all units within this profile. This command is a combined/wizard command in the sense that you could "
		...
		help.addComment("Move a unit from this profile to another profile within the same unittype. The unit will keep all unit parameters.");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<unitid>", "A valid unit id.");
	...
	private HelpGroup unittypeHelpBuilder() {
		...
		help.addArgument("[[!]<unittype-parameter-name-pattern>]", getPatternComment("unittype-parameter"));
		help.addExamples("listparams", "listparams DeviceInfo", "listparams ^Device.Device.*");
		hg.addHelp(help);
		...
		argCom += "in the device. Max length: 255 characters. Tip: Fusion TR-069 Server, discovery mode. ";
		argCom += "If changing: A valid unittype parameter name.";
		help.addArgument("<unittype-parameter-name>", argCom);
		argCom = "The flags D,A,C,S,I,M are optional, R, RW or X is mandatory. The order of the flags are not important.\n";
		argCom += "\t[A]lwaysRead   - The value will always be read from the device. Cannot be combined with I, RW or X.\n";
		argCom += "\t[B]ootRequired - If such a parameter is sent/set to the device, an Reboot will follow immediately after\n";
		argCom += "\t[C]onfidential - The value will probably be sent from the device as \"\", usually used for passwords. This flag will ";
		argCom += "suppress a warning that is logged when database and device differ while it was expected to be equal. The logs will obscure the value, to minimize exposure.\n";
		argCom += "\t[D]isplayable  - The value will be shown as a column in the search page of Fusion Web. Cannot be combined with I and C.\n";
		argCom += "\t[S]earchable   - Possible to search for this particular parameter in Fusion Web, using the Advanced Search.\n";
		argCom += "Web in the Unit Parameter page. After inspection mode is finished, the inspection parameters read will be deleted. Useful for short-lived parameters.\n";
		argCom += "\t[M]onitor      - The value will be sent to Fusion Syslog Server, and can be processed into a TR-report (given the appropriate ";
		argCom += "parameters are set to M). The parameters are stored to database only if combined with A flag.\n";
		argCom += "\t(R)eadOnly     - The parameter can only be read from the device, never written to the device.\n";
		argCom += "\t(R)ead(W)rite  - The parameter can be written to the device. Only read from device if combined with I.\n";
		argCom += "\t(X) system     - The parameter is only for internal use, never sent to the device. You can create as many X parameters as you ";
		...
		help.addComment("Delete a unitttype parameter. Cannot be done unless the parameter is not used in any unit, profile, job, group.");
		...
		help.addComment("Move a unit to another unittype. Requires that the target unittype has defined all unittype parameters found for this unit's parameters and that the profile exists. ");
		help.addOption(getUseContextHelpOption());
		...
		help.addComment("Cleanup of old and unused system parameters. Will never delete a system parameter that is in use.");
		help.addOption(getUseContextHelpOption());
		hg.addHelp(help);
		...
		help = new Help("importfile <filename> <filetype> <version> <timestamp> <targetname> <description>");
		help.addComment("Add/import file to Fusion filestore. ");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<filename>", "Filename of the file (found in the filesystem). Use / as directory path delimiter.");
		argCom = "SOFTWARE      - CPE image files, used in software upgrade\n";
		argCom += "TR069_SCRIPT  - A CPE script/config file, used in TR-069 Script Upgrade. A Target filename must be specified\n";
		argCom += "SHELL_SCRIPT  - A Fusion script file, used in Fusion Shell, Shell Jobs, Triggers and Syslog Events\n";
		argCom += "TELNET_SCRIPT - A Telnet script file, used in Telnet Job\n";
		argCom += "UNITS         - A unit file, first column in file must be a valid unit-id, used in Trigger Script execution\n";
		argCom += "MISC          - A file, no particular use-case\n";
		help.addArgument("<filetype>", argCom);
		argCom = "Extremely important if importing a SOFTWARE or a TR069_SCRIPT. It MUST be the version number reported from the software/script when installed in ";
		argCom += "the device. Otherwise it will create a download loop! For other file types this is not as important.";
		help.addArgument("<version>", argCom);
		help.addArgument("timestamp", "Specify a timestamp (format is : yyyyMMdd-HH:mm:ss) or set to NULL (which will translate into current timestamp)");
		help.addArgument("targetname", "Optional (set to NULL) for all file types except TR069_SCRIPT, in which case the targetname is used to distinguish between multiple Script Upgrade.");
		help.addArgument("ower", "Owner of file. Can only be set if you're admin, if not it will automatically (and silently) be set to your username");
		...
		help.addExamples("importfile firmware.bin SOFTWARE 5.0.3-alfa NULL NULL myuser \"A firmware for the NPA201E\"");
		help.addExamples("importfile voip.zip TR069_SCRIPT 1 NULL /opt/voip.zip NULL \"A Zip-file download to the CPE\"");
		hg.addHelp(help);

		help = new Help("exportfile <filename>");
		help.addComment("Export file from Fusion filestore. ");
		help.addOption(getUseContextHelpOption());
		argCom = "A filename from Fusion filestore. If the file already exists it will create a directory named ";
		argCom += "after the unittype, and place the file there. The import command will always search for such a location first ";
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("<profilename>", "Any string, enclose arguments with spaces using double quotes.");
		help.addExamples("setprofile Default");
		hg.addHelp(help);

		help = new Help("delprofile <profilename>");
		help.addComment("Delete a profile. Not allowed if there are units within this profile or if groups are connected to this profile.");
		help.addOption(getUseContextHelpOption());
		...
		help.addArgument("[PARENT-FIRST|PARENT-LAST]", "Defines the ordering of the list. This is usually not interesting - mostly used for -export/import options of this shell");
		help.addExamples("listgroups", "listgroups \"All units\"");
		hg.addHelp(help);

		help = new Help("setgroup <groupname> <parent-groupname>|NULL <description> <profile-name>|NULL");
		help.addComment("Add/change a group.");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<groupname>", "Adding: Any string. Enclose with double quotes if it contains spaces. Changing: Valid group name.");
		argCom = "A group can be a child of another group. In that case it will inherit the group parameters from the parent group. ";
		argCom += "The group parameters will then be ANDed together to form one or more criteria for the group search.";
		...
		argCom += "If a parent group is specified, the profile setting of that group will override this.";
		help.addArgument("<profile-name>|NULL", argCom);
		//		argCom = "If you like to create a time-rolling group (other terms might be 'time' and 'monitor' groups), you must specify a time parameter. ";
		//		argCom += "The effect of this is that Fusion Core Server will change this group's time parameter criteria on a periodic interval, ";
		//		argCom += "which in turn is extremely useful for a special Syslog Event copying group parameters to a unit. The whole idea is to be able  ";
		//		argCom += "to monitor a certain event as it varies over time. In that case, specify a valid unit type parameter. Please specify a System parameter name, ";
		//		argCom += "run 'help setparam' and read about the X flag and the name convention. Otherwise set it to NULL";
		//		help.addArgument("<time-param-name>|NULL", argCom);
		//		argCom = "The time-format can be any string, but some characters (and group of characters) take on a special meaning. A format like ";
		//		argCom += "'yyyyMMddHH' will populate (through Fusion Core Server) the time parameter with something like '2010111814'. The formatting ";
		//		argCom += "rules are described in SimleDateFormat (search on the internet). If time parameter is set and this format set to NULL, the ";
		//		argCom += "default value will be 'yyyyMMdd'.";
		//		help.addArgument("<time-format>|NULL", argCom);
		//		argCom = "When Fusion Core Server sets this value, it will subtract the offset (seconds) from current time. ";
		//		argCom += "If timeparameter is set and this offset set to NULL, the default value will be 0.";
		...
		help.addArgument("[DEP-FIRST|DEP-LAST]", "Defines the ordering of the list. This is usually not interesting - mostly used for -export/import options of this shell");
		help.addExamples("listjobs", "listjobs Upgrade");
		hg.addHelp(help);

		String fileArg = "<file-version>|NULL";
		help = new Help("setjob <jobname> <jobtype> REGULAR|DISRUPTIVE <groupname> <parent-jobname>|NULL <description> " + fileArg
				+ " <unconfirmed-timeout> <stop-rules>|NULL [<repeat-count> <repeat-interval-sec>]");
		help.addComment("Make a job. There is a variety of jobtypes to be made. Using job to perform provisioning is to leverage the full force of Fusion, instead"
				+ "of the simple Profile/Unit provisioning.");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<jobname>", "Adding: Any string. Enclose names with double quotes if it contain spaces");
		argCom = "CONFIG       -  Change parameters/configuration/settings on CPE\n";
		argCom += "SOFTWARE     -  Upgrade/downgrade software/firmware on CPE\n";
		argCom += "TR069_SCRIPT -  Upload a proprietary script/config-file/etc to the CPE - will be executed on CPE\n";
		...
		argCom += "KICK         -  Will issue a 'kick' to the CPE, which will trigger a regular provisioning cycle\n";
		argCom += "TELNET       -  Will execute a Telnet-script on the CPE (make sure Telnet-parameters are defined on unit/profile)\n";
		...
		help.addArgument("<groupname>", "A valid group name, only units that matches this group parameters will be able to run the job");
		help.addArgument("<parent-jobname>|NULL", "If the job depends upon another job, set the parent job here. Otherwise set to NULL");
		help.addArgument("<description>", "A non-essenital (for Fusion) description.");
		argCom = "The file to be used in the job (if the job type require a file). The version specified will be used to retrieve the file of the correct type:\n";
		...
		help.addArgument("<unconfirmed-timeout>", "If a device doesn't reconnect after a job within this specified number of seconds, the job for this unit is considered 'unconfirmed failed'");
		argCom = "Stop rules tells a job to 'pause' or 'stop' when a certain condition occur. A stop rule can be specified on this ";
		...
		argCom += "\tn : counter\n";
		argCom += "\tRule example 1: a10/1000 : Will stop the job if 10 out of the last 1000 unit jobs has any failure\n";
		argCom += "\tRule example 2: u5,n1000 : Will stop the job if 5 unconfirmed failure occurs or if 1000 unit jobs has been executed.\n";
		help.addArgument("<stop-rules>|NULL", argCom);
		help.addArgument("[<repeat-count>]", "Optional. Set to a positive number to repeat a job. Normal behavious is to never repeat a job. This is useful for monitoring jobs.");
		help.addArgument("[<repeat-interval-sec>]",
		...
		help.addExamples("setjob Upgrade SOFTWARE DISRUPTIVE \"All units\" NULL \"An upgrade job\" 0.9-alfa 600 \"u5,a100\" NULL NULL");
		help.addExamples("setjob Test CONFIG REGULAR \"All units\" NULL \"A config job\" 1.2.3-alfa 600 a100 NULL NULL");
		hg.addHelp(help);
		...
		help = new Help("setsyslogevent <id> <name> <group-name>|NULL <expression> <store-policy> <script-name>|NULL <delete-limit>|NULL <description>");
		help.addComment("Add/change a syslog event. A syslog event is an event which is trapped on the syslog server if a certain syslog message arrive.");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<id>", "Adding: The id must be a number larger or equal to 1000. Changing: A valid syslog event id.");
		help.addArgument("<name>", "A non-essential name (for Fusion) of the syslog event.");
		help.addArgument("<group-name>|NULL", "If specified, the syslog-event will only match the units within the group");
		help.addArgument("<expression>", "This is the expression with which syslog messages will be matched. Syntax:\n " + expressionSyntax
				+ "The matching is partial - you don't need to match the entire syslog message with the "
				+ "expression. A message will perform/execute the first event it matches, so avoid setting up events that match on the same " + "syslog message."
				+ "Check out the matching using the command 'syslog sm=<expression>'");
		argCom = "Storepolicy defines what this syslog event will store the message. You may choose one out of these policies:\n";
		argCom += "\tSTORE - This is the default task for all messages that arrive to the syslog server\n";
		argCom += "\tDISCARD - Discards the message, a useful event to throw away garbage messages.\n";
		argCom += "\tDUPLICATE - Duplicate messages will only be printed once every 60 minutes (count will be updated)\n";
		help.addArgument("<store-policy>", argCom);
		help.addArgument("<script-name>", "If specified a script will be run upon the Syslog Event");
		help.addArgument("<delete-limit>", "If specified, deletion limit will override the default severity limit specified in the Fusion Core Server configuration. It may delay or hasten "
				+ "the deletion of specific type of syslog message. Default value is 0 (which is to say 'use the default severity limit')");
		help.addArgument("<description>", "A non-essential (to Fusion) description of the syslog event.");
		help.addExample("setsyslogevent 1000 SipRegFailed-monitor NULL \"reg failed\" STORE NULL NULL \"A monitor over sip registrations that failed\"");
		help.addExample("setsyslogevent 2000 BlockedMsg-drop NULL \"gw: Blocked message\" DISCARD NULL 0 \"Will drop unneccessary messages from the device\"");
		help.addExample("setsyslogevent 3777 Script-runner MyGroup \"DNS Error\" DUPLICATE setdebugflagonunit.xss 15 \"Will force a system-debug flag to 1 using a script\"");
		hg.addHelp(help);
		...
		if (ACSVersionCheck.scriptExecutionSupported) {
			...
			help.addOption(getListContextHelpOption());
			help.addOption(getOrderHelpOption());
			help.addArgument("match-expression", "A regular expression string which will be matched against script-name, script-arguments and error-message");
		...
		if (ACSVersionCheck.heartbeatSupported) {
			...
			help.addOption(getListContextHelpOption());
			help.addOption(getOrderHelpOption());
			help.addArgument("match-expression", "A regular expression string which will be matched against name, group-name and expression");
			help.addExamples("listheartbeats", "listheartbeats Test");
			hg.addHelp(help);

			help = new Help("setheartbeat <name> <expression> <group> <timeout>");
			help.addComment("Add/Change a \"Missing Heartbeat Detection\". By defining a heartbeat, Fusion Core server will generate "
			...
			help.addArgument("<expression>", "An expression used to match against the syslog message. If no units in the group have "
					+ "sent messages that matches this expression within the timeout period, then the heartbeat is considered \"missing\" " + "and a syslog message will be generated. Syntax:\n "
					+ expressionSyntax + "To improve Fusion reports, the following expressions can be used - and they will be caught by the report system:\n"
					+ "\t^ProvMsg\tMatches the ProvisioningHeartbeat\n" + "To test the expression and if it works, test it with the 'syslog sm=<expression>'\n");
			help.addArgument("<group>", "The group name - the group should match on those units which emits \"heartbeat\" messages");
			help.addArgument("<timeout>", "The timeout in number of hours. Must be a number between 1 and 48.");
		...
		if (ACSVersionCheck.triggerSupported) {
			...
			help.addOption(getListContextHelpOption());
			help.addOption(getOrderHelpOption());
			help.addArgument("match-expression", "A regular expression string which will be matched against trigger-name, description, profile-name and group-name");
			help.addArgument("[PARENT-FIRST|PARENT-LAST]", "Defines the ordering of the list. This is usually not interesting - mostly used for -export/import options of this shell");
			...
			help.addArgument("<active>", "Activate or de-activate triger. Can be either \"true\" or \"false\"");
			//			help.addArgument("<group-name>", "A group name which the trigger should be based upon. May be set to NULL");
			help.addArgument("<evaluation-period-minutes>", "A value from 15 to 120. This is the time period for which the trigger may be fulfilled.");
			help.addArgument("<notify-interval-hours>", "A value from 1 to 168. This is the minimum interval between each ALARM or REPORT. May be set to NULL.");
			help.addArgument("<script>", "A script filename has to be set if <action> is SCRIPT. Otherwise NULL.");
			help.addArgument("<parent-triggername>", "A parent trigger. May be set to NULL.");
			help.addArgument("<to-list>", "A list of comma separated email-addresses. May be set to NULL");
			help.addArgument("<syslog-event-id>", "Set only if <type> is BASIC, otherwise NULL or skipped. A syslog event id which the trigger should be based upon.");
			help.addArgument("<no-events-total>", "Set only if <type> is BASIC, otherwise NULL or skipped. A value larger than 0. The number of events which is needed to fulfill the trigger.");
			help.addArgument("<no-events-pr-unit>",
			...
			help.addArgument("<no-units>", "Set only if <type> is BASIC, otherwise NULL or skipped. A value larger than 0. The number of units needed to fulfill the trigger");
			help.addExample("settrigger CompDNS \"ALARM on DNS troubles\" COMPOSITE REPORT true 15 1 NULL NULL senior@lab.com NULL NULL NULL NULL");
			help.addExample("settrigger BasicDNS \"ALARM on DNS troubles\" BASIC ALARM true 15 1 NULL CompDNS junior@lab.com 3100 1 1 1");
			hg.addHelp(help);
		...
		//		help = new Help("makegroupmonitor <syslog-event-id> <syslog-expression> <group-name> <time-parameter> <time-rolling-format>|NULL");
		//		help.addComment("This command is a combined/wizard command in the sense that you could achieve the same as this command does by running 2 or 3 other commands. "
		//				+ "The purpose of the command it to create a monitoring of a syslog message and to set up the correct syslog event and the groups needed to monitor. "
		//				+ "It will actually also create the unit type parameter if necessary. The group that is created is time-rolling/monitor group, and the syslog event "
		//				+ "is has a GROUPSYNC task. Together with Fusion Core Server (which must produce Group report) and Fusion Web (which shows the results) this will enable a "
		//				+ "powerful feature to monitor any syslog message expression that the device might send. However, creating a lot of syslog events will reduce the performance " + "of the syslog server.");
		//		help.addOption(getUseContextHelpOption());
		//		help.addArgument("<syslog-event-id>", "The id must be a number larger or equal to 1000.");
		//		help.addArgument("<syslog-expression>", "This is the expression with which syslog messages will be matched. The mathing is performed by a regular expression matcher "
		//				+ "(check internet resources on this topic). The matching is partial - you don't need to match the entire syslog message with the "
		//				+ "expression. As of today, a message will perform the first event it matches, so avoid setting up events that match on the same " + "syslog message.");
		//		help.addArgument("<group-name>", "Any string. Enclose with double quotes if it contains spaces.");
		//		argCom = "If you like to create a time-rolling group (other terms might be 'time' and 'monitor' groups), you must specify a time parameter. ";
		//		argCom += "The effect of this is that Fusion Core Server will change this group's time parameter criteria on a periodic interval, ";
		//		argCom += "which in turn is extremely useful for a special Syslog Event copying group parameters to a unit. The whole idea is to be able  ";
		//		argCom += "to monitor a certain event as it varies over time. In that case, specify a valid unit type parameter. Please specify a System parameter name, ";
		...
		//		help.addExamples("makegroupmonitor 1000 \"reg failed\" \"All units\" System.X_COMPANY-COM.Monitor.SipRegFailed NULL");
		//		help.addExamples("makegroupmonitor 1000 \"reg failed\" \"All units\" System.X_COMPANY-COM.Monitor.SipRegFailed yyyyMMddHH");
		//		hg.addHelp(help);

		//		help = new Help("enabletr069report <reporttype> <group-name> <interval> [<uploadURL>|NULL <downloadURL>|NULL <ping-host>|NULL]");
		//		help.addComment("This command is a combined/wizard command in the sense that you could achieve the same as this command does by running "
		//				+ "2-8 other commands. The purpose is to enable a TR-069 report to get information from the devices regarding a number of interesting metrics. "
		//				+ "Make sure you enable the report in Fusion Core Server configuration in addition to this operation. Each monitoring operation "
		//				+ "may require up till 4 jobs running (GatewayTR requires 4 jobs to run, the others 1 job). The GatewayTR monitoring is also "
		//				+ "demanding on the network, since they can run upload-test and download-test. All in all, monitoring using TR-069 metrices "
		//				+ "cost resources of your system. It's therefore a good advice to not set up all units to do monitoring, and not to do it all the time.");
		//		help.addOption(getUseContextHelpOption());
		//		help.addArgument("<reporttype>", "The reporttype can be one out of three:\n" + "\tHardwareTR   - A report for temperature, cpu-usage, memory, etc\n"
		//				+ "\tGatewayTR    - A report for upload/download speed, ping-time, WAN-uptime\n" + "\tVoipTR       - A report for voip-quality, call-length, loss, etc\n"
		//				+ "Note that most devices probably wont support these metrices, but some will support some of it. A very few might support all of them.");
		//		help.addArgument("<group-name>", "A valid group name within this unittype. One piece of advice: Do not make the group too large (test with 100, "
		//				+ "then 1000 units), it will put a huge strain on the system.");
		//		help.addArgument("<interval>", "An interval in minutes. Do not set it too low (test with 1440, then 60 minutes), it will put a huge strain on the system.");
		//		help.addArgument("<downloadURL>|NULL", "If you want to run GatewayTR monitoring you can offer a downloadURL. This URL will be executed from the device "
		//				+ "to test the download-speed. You may skip it (setting to NULL).");
		//		help.addArgument("<uploadURL>|NULL", "If you want to run GatewayTR monitoring you can offer a uploadURL. This URL will be executed from the device "
		//				+ "to test the upload-speed. You may skip it (setting to NULL).");
		//		help.addArgument("<pingHost>|NULL", "If you want to run GatewayTR monitoring you can offer a ping host. This hostname will be used from the device "
		//				+ "to test the ping times. You may skip it (setting to NULL).");
		//		help.addExample("enabletr069report HardwareTR monitor-group 60");
		//		help.addExample("enabletr069report GatewayTR monitor-group 60 http://xaps.comp.com/xapstr069/file/uptest http://xaps.comp.com/xapstr069/file/downtest xaps.com.com");
		//		hg.addHelp(help);
		...
		help.addOption(getListContextHelpOption());
		help.addOption(getOrderHelpOption());
		help.addArgument("match-expression", "A regular expression string which will be matched against script, args and request-Id");
		...
		help.addArgument("<request-id>", "A request id, to identify the source/requestor of the script execution");
		...
		help.addArgument("<method>|NULL", "Optional. Could be set to VALUE, ATTRIBUTE or FILE. Setting it to NULL will match all (methods)");
		help.addArgument("<parameter-filter>|NULL", "Optional if the previous argument is specified. Could be set to a part of a parameter name. Setting it to NULL will match all (parameters)");
		help.addArgument("<tag-filter>|NULL",
				"Optional if the previous argument is specified. Could be set to a series of strings enclosed in square brackets [ ] to match tags. Setting it to NULL will match all (tags)");
		help.addExample("listtc NULL NULL NULL");
		help.addExample("listtc VALUE WANConnection \"[READONLY][GENERATED]\"");
		hg.addHelp(help);
		...
		help.addComment("Validate the unittype parameter flags according to the TR-069 datamodel (TR-098, TR-104 and TR-181). It will only list those entries which violates the datamodel");
	...
	private HelpGroup rootHelpBuilder() {
		...
		help.addOption(getListContextHelpOption());
		help.addOption(getOrderHelpOption());
		help.addArgument("[<search-value>]", "Optional. If this is the only argument, it will search among all unit parameters and unit-ids. ");
		help.addExamples("listunits AABBCC0123", "listunits \"John Doe\"");
		hg.addHelp(help);

		help = new Help("setunittype <unittype-name> TR-069|N/A (<vendor> <description>)|<unittype-file> ");
		help.addComment("Add/change a unittype. No parameters are added.");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<unittype-name>", "Any string if adding. If changing it must be a valid unittype name.");
		...
		argCom = "A file in the filesystem (cannot retrieve from Fusion filestore). Use / as directory path delimiter. The file must ";
		...
		argCom = "Delete a unittype. The command will not succeed if profiles/groups/jobs/software/files exists in this unittype. ";
		argCom += "For a complete delete procedure, use the -delete option when executing xapsshell.jar";
		help.addComment(argCom);
		help.addOption(getUseContextHelpOption());
		...
		help.addOption(getUseContextHelpOption());
		help.addOption(getOrderHelpOption());
		help.addArgument("[staging]", "Optional. If set to 'staging' all system parameters regarding staging will be shown. Staging is a special concept, usually only for CPE vendors");
		...
		help.addOption(getUseContextHelpOption());
		help.addArgument("<username>", "Any string if adding. If changing it must be a valid user name.");
		...
		argCom += "Thus if the password is forgotten, it cannot be retrieved. However, using this command it can be ovewritten ";
		argCom += "without any problem. The user/password information is needed to login to Fusion Web and Fusion Web Services.";
		help.addArgument("<secret>", argCom);
		argCom = "A comma-separated list of pages that the user can access in Fusion Web. The valid pages are\n";
		...
		argCom = "Can be set to \"true\" or \"false\". If set to \"true\" the user will have access to all unittypes and profiles ";
		argCom += "regardless of any permissions added to this user later. If set to \"false\" the user will have no permissions ";
		argCom += "unless explicitely given permissions (using setperm). The flag cannot be set to true unless the user running the command ";
		...
		help.addOption(getUseContextHelpOption());
		argCom = "Must be a valid username. If 'admin' user is deleted, a default 'admin' user will still ";
		...
		argCom = "List permissions in Fusion. Each permission is attached to a user and controls which unittypes/profiles are allowed for that user. ";
		argCom += "If a permission for a unittype is created with NULL as profile value, no other permissions for that unittype can be created.";
		help.addComment(argCom);
		help.addOption(getUseContextHelpOption());
		help.addOption(getOrderHelpOption());
		...
		help.addArgument("<profile-name>|NULL", "If NULL, then all profiles within the unittype is permitted. Else only the specified profile is permitted.");
		...
		help.addComment("List certificates in Fusion. Shows the complete certificate string in both encrypted version and clear-text");
		help.addOption(getUseContextHelpOption());
		help.addOption(getOrderHelpOption());
		hg.addHelp(help);

		help = new Help("setcertificate <name> <certificate>");
		help.addComment("Add/change a certificate. Will not allow all kinds of changes (going from production to test certificate is not allowed)");
		help.addOption(getUseContextHelpOption());
		help.addArgument("<name>", "The name of the certificate is not important. Fusion will recognize the type no matter the name");
		argCom = "A hexadecimal 256-char string which is the certificate.Certificates can only be retrieved from Ping Communication. ";
		...
		help.addExample("setcertificate Test ");
		hg.addHelp(help);
	...
	private HelpGroup genericHelpBuilder() {
		...
		help.addComment("Change to another context. The context change is fairly robust, so you can make context changes directly from, say a unit context  to "
				+ "a group context, if only the group specified is within the same unit type.");
		String context = "Specify context to change to.\n" + "Syntax: .. or /(<type>:<contextname>/)*\n"
		...
		help.addExamples("cc ..", "cc /", "cc /ut:TestUnittype/pr:TestProfile", "cc \"/ut:Test Unit Type/\"", "cc /gr:MyGroup");
		hg.addHelp(help);

		help = new Help("unit [<search-value>]");
		help.addComment("Switch context to a particular unit. If more than 1 unit is found, all the units are "
		...
		help.addOption(getUseContextHelpOption());
		help.addOption(getOrderHelpOption());
		argCom = "Optional. Default search is for everything the last 24h, but only return the 100 most recent entries. If search argument contains whitespace, enclose with double quotes. The search argument syntax is:\n"
		...
		argCom += "fn=.+      - Facility-name (ex: Device-Local<number>, Fusion TR069, Fusion Web, etc)\n";
		...
		argCom += "Some fields are marked with (expr*). These fields can be specifed using the following syntax:\n" + expressionSyntax;
		argCom += "To search within a specific unittype, profile or unit, change to the appropriate context and search.\n";
		help.addArgument("[<search-args>]", argCom);

		argCom = "Optional. Default listing is timestamp, severity, facility, message, ipaddress, unitid, profile and unittype. The list argument syntax is:\n";
		...
		help.addComment("A call to another script. Possible to pass context-information and variables. ");
		context = "Specify context to run the script within. Default is to run in the same context as the callee.\n" + "Syntax: -c/(<type>:<contextname>/)*\n"
				+ "Types are: 'ut' (Unit Type), 'pr' (Profile), 'un' (Unit), 'gr' (Group), 'jo' (Job) or " + "'up' (Unit Type Parameter). Use a single '/' to specify root. If context name contains "
				+ "whitespace, enclose the entire option with quotes: \"-c/ut:Test UT/\".";
		help.addOption("-u", context);
		String optCom = "Specify variables to pass to the script. The variables are passed using the " + "variable name or a value\n" + "Syntax: -v(<var1>)(,<var2>)*\n"
		...
		argCom += "Fusion script files, the context cannot be root. In case a script has the same name ";
		argCom += "in the filesystem and in Fusion, the Fusion script file is used. For filesystem scripts ";
		...
		argCom = "Setting a variable which may be read in this session. The variables will not be passed ";
		argCom += "to a new script when using call, then you need to use -v option in the call command. ";
		argCom += "To reference a variable use ${name}. ";
		help.addComment(argCom);
		help.addArgument("<name>", "Any name, but cannot start with a number, or an undescore or contain whitespace.");
		help.addArgument("<expression>", "A Javascript expression. Use single quotes to specify a string value.");
		...
		argCom += "Fusion script files, the context cannot be root. In case a script has the same name ";
		argCom += "in the filesystem and in Fusion, the Fusion script file is used. For filesystem files ";
		argCom += "use / as directory path delimiter.";
		help.addArgument("<filename>", argCom);
		...
		help.addComment("Import a unittype from a set of files previously exported using exportunittype command");
		help.addArgument("<unittypename>|ALL", "Specify the unittype name or ALL for all unittypes. In both cases the "
				+ "command expects a directory with this name to exist in working directory and it must contain " + "the files exported from exportunittype command");
		help.addExamples("unittypeimport ALL", "unittypeimport NPA201E");
		hg.addHelp(help);

		help = new Help("unittypeexport <unittypename>|ALL");
		help.addComment("Export a unittype to a set of files in a directory with the same name as the unittype");
		help.addArgument("<unittypename>|ALL", "Specify the unittype name or ALL for all unittypes. In both cases the "
		...
		help.addComment("Echos the <text> argument to screen. Will evaluate any references to variables");
		...
		argCom += "Use 'generic' to get list of generic commands. To get detailed help, specify a command ";
		argCom += "name. There is no need to specify the whole command name, just specify enough to make ";
		...
		help.addArgument("[<directory>]", "Optional. Default is the working directory. Use / as directory path delimiter.");

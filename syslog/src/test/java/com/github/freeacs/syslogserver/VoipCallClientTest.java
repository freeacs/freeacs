...
public class VoipCallClientTest extends SyslogClientTest implements Runnable {
  ...
  public VoipCallClientTest(String mac, String hostname, boolean realtime) {
    ...
    if (!realtime) startTms = System.currentTimeMillis() - 7l * 24l * 3600l * 1000l;
    else startTms = System.currentTimeMillis();
  }
  ...
  public void run() {
    ...
    while (true) {
      try {
        ...
        if (!initiallyRealtime && realtime) return;
        ...
        for (int i = 0; i < numberOfMOS; i++) {
          ...
          if (problemCall && i == (numberOfMOS - 4)) MOS -= 150;
          if (MOS > 436) MOS = 436;
          if (MOS < 100) MOS = 100;
          tms = sleep(tms, 5000);
        }
        ...
        if (jitterMax <= jitterAvg) jitterMax = jitterAvg + 1;
        ...
        msg =
            makeMessage(
                ...
                "QoS report for channel 0: MOS: "
                    ...
                    + "ms), Audio (Valid: "
                    + callLength
                    + " Concealed: 571ms), loss "
                    + lossPercent
                    + "%");
        send(msg, hostname);
        if (!realtime) tms += (3600 * 1000 * 6) + random.nextInt(3600 * 1000 * 3);
      } catch (Throwable t) {

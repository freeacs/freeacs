...
xAPS.createModule("unit.inspection",function($this,$super,$){
	...
	function executeModeAndStateMonitoring() {
		...
		try {
			...
			if (mode && state) {
				var url = "web?page="+$this.settings.inspectionPageId+"&unit=" + $this.settings.unitId + "&mode=" + mode + "&state=" + state + "&timestamp="+new Date().getMilliseconds();
				...
				var method = function(data) {
	                ...
	                if (cmd == "relo") {
	                    url = "web?page="+$this.settings.unitPageId+"&unit=" + $this.settings.unitId;

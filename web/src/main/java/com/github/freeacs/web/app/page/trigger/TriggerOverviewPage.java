...
public class TriggerOverviewPage extends AbstractWebPage {
	...
	public void process(ParameterParser params, Output outputHandler, DataSource xapsDataSource, DataSource syslogDataSource) throws Exception {
		...
		if (unittype != null) {
			...
			if (inputData.getAction().hasValue(DELETE_ACTION)) {
				...
				outputHandler.setRedirectTarget(Page.TRIGGEROVERVIEW.getUrl() + "&action=edit&triggerId=" + inputData.getTriggerId().getValue());
	...
	private void setOutputDataForEditTriggerMode(Output outputHandler) {
		...
		outputHandler.getTemplateMap().put("syslogEvents", triggerHandler.getSyslogEventDropdown(trigger.getSyslogEvent()));
		outputHandler.getTemplateMap().put("typeTrigger", triggerHandler.getTriggertypeDropdown(trigger.getTriggerType()));
		outputHandler.getTemplateMap().put("evalPeriodMinutes", triggerHandler.getNewEvalPeriodDropdown(trigger.getEvalPeriodMinutes()));
		outputHandler.getTemplateMap().put("notifyIntervalHours", triggerHandler.getNotifyIntervalHoursDropdown(trigger.getNotifyIntervalHours()));
		outputHandler.getTemplateMap().put("parentTrigger", triggerHandler.getTriggerParentDropdown(trigger));
		outputHandler.getTemplateMap().put("notifyType", triggerHandler.getNotifyTypeDropdown(trigger.getNotifyType()));
		outputHandler.getTemplateMap().put("scriptFiles", triggerHandler.getScriptFileDropdown(trigger.getScript()));
	...
	private void setOutputDataForCreateTriggerMode(Output outputHandler) {
		...
		outputHandler.getTemplateMap().put("evalPeriodMinutes", triggerHandler.getNewEvalPeriodDropdown(null));
		outputHandler.getTemplateMap().put("notifyIntervalHours", triggerHandler.getNotifyIntervalHoursDropdown(null));
		outputHandler.getTemplateMap().put("parentTrigger", triggerHandler.getTriggerParentDropdown(null));

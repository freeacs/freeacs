 ...
 * 
 * It also maintains a set of integers, for counting errors and warnings, and total entries.
 * 
 * Of special interest is the addRecord method, that is responsible for updating the numbers according to the severity level of the added record.
 * 
 * @author Jarl Andre Hubenthal
 *
 */
public class RecordUIDataSyslogSumFromReport implements Comparable<RecordUIDataSyslogSumFromReport> {
	...
	/** The records. */
	private List<RecordUIDataSyslogFromReport> records = new ArrayList<RecordUIDataSyslogFromReport>();
	...
	/**
	 * This method adds a record to an internal list, and updates three integers, warnings/errors/total.
	 * 
	...
	public String getRowBackgroundStyle() {
		try {
			...
			if(warnings>0)
				score = score-30;
			rowBackgroundStyle = new AbstractWebPage.RowBackgroundColorMethod().exec(Arrays.asList(score+""));

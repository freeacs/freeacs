...
public class CryptoPage extends AbstractWebPage {
        ...
        if (inputData.getAction().isValue("delete")) {
          try {
            ...
            session.setAttribute(
                "info", "Successfully deleted Certificate [" + certToEdit.getId().toString() + "]");
        ...
        try {
          cert =
              new Certificate(
                  ...
                  inputData.getCertificate().getFileAsString());
          try {
            acs.getCertificates().addOrChangeCertificate(cert, acs);
            params
                .getSession()
                .setAttribute("info", "Successfully added Certificate [" + cert.getId() + "]");
            outputHandler.setDirectToPage(Page.CERTIFICATES); // Avoid postback warning
            return;
          } catch (
              IllegalArgumentException
                  certificateExists) { // Thrown by addOrChange if cert type exists
            ...
            if (valid) {
              ...
              if (certToDelete.isTrial()) {
                ...
                params
                    .getSession()
                    .setAttribute(
                        "info", "Successfully updated Certificate [" + cert.getId() + "]");
                outputHandler.setDirectToPage(Page.CERTIFICATES); // Avoid postback warning
                return;
              }
            }
  ...
  public class IsModuleValidMethod implements TemplateMethodModel {
    ...
    public Boolean exec(List args) throws TemplateModelException {
      ...
      if (cert == null) throw new IllegalArgumentException("Certificate " + id + " was not found");

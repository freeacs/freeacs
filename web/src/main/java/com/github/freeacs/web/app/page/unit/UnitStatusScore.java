...
public class UnitStatusScore {
	 ...
	 * @param isWithinServiceWindow the is within service window
	 */
	public void setHardwareEffect(List<RecordUIDataHardware> hardwareReport,boolean isWithinServiceWindow) {
		for(RecordUIDataHardware uiDataRecord:hardwareReport){
			if(uiDataRecord.getBootTotal()>0){
				if(uiDataRecord.getBootWatchdogCount().get()>0)
					this.hardwareEffect += (isWithinServiceWindow?HARDWARE_WITHIN_SERVICE_WINDOW_PENALTY:HARDWARE_OUTSIDE_SERVICE_WINDOW_PENALTY);
	...
	public Double getStatus(){
		double status = (double)TOP_STATUS-this.getServiceWindowEffect()-this.getTotalScoreEffect()-this.getVoipLineEffect()-hardwareEffect-syslogEffect;
		if(status<0)
			return 0d;
		return status;
	}

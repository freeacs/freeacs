...
//		System.out.println("Template set");
	...
	/* Should output an scriptexecution, if an scriptexecution is chosen. If not a default "blank" scriptexecution must be the output
	 * Will always output a list of all possible scriptexecutions within this unittype.
	 */
	private void output(Output outputHandler, Unittype unittype, Script script) throws Exception {
		...
		if (script != null) // Execute on ScriptExecutionPage has been clicked
			selectedFile = script.getScript();
		else if (inputData.getId().getInteger() != null) { // Clicked on a script execution (in history-list)
...
//							scriptArg.getEnumDropDown().setSelected(new Enumeration(in.getString(), in.getString()));
		...
		} else if (inputData.getFormSubmit().getValue() == null && inputData.getFileId().getInteger() != null) { // Clicked on "Execute"-button i File page
			selectedFile = unittype.getFiles().getById(inputData.getFileId().getInteger());
			script = parse(selectedFile, unittype);
		}

		if (script != null && script.getScriptArgs() != null && script.getScriptArgs().size() > 0)
		...
		fmMap.put("scripts", InputSelectionFactory.getDropDownSingleSelect(inputData.getFileId(), selectedFile, allScriptFiles));
		
		/* Output for the event list */
		List<ScriptExecution> execList = scriptExecutions.getExecutions(unittype, new Date(System.currentTimeMillis() - 24 * 3600 * 1000), null);
	...
	private Script parse(File scriptFile, Unittype unittype) throws Exception {
		...
		List<ScriptArg> scriptArgList = new ArrayList<ScriptArg>();
		int index = 0;
		String title = null;
			...
			if (inputData.getFileId().getInteger() != null) {
				try {
					...
					if (typeCheckOk) {
						...
						outputHandler.getServletResponse().sendRedirect(Page.SCRIPTEXECUTIONS.getUrl() + "&info=Script execution initiated successfully");
					}
					return script;
				} catch (Throwable ex) {
					fmMap.put("error", "Could not execute script (" + ex.getClass() + " : " + ex.getMessage() + ")");
				}
			} else {

 ...
 * 
 * Contains a static add method <code>addRecord(RecordUIDataHardware record)</code>.
 * 
 * Maintains a set of booleans for memory status and an integer for number of boots.
 * 
...
public class RecordUIDataHwSum{
	...
	public Long getCpeUptimeAvg(){
		...
		for(RecordUIDataHardware rec:records){
			if(rec.getTms().getTime()>millis && rec.getCpeUptimeAvg().get()!=null && rec.getCpeUptimeAvg().get()>0){
				millis = rec.getTms().getTime();
				record = rec;
			}
		}
		return record!=null?record.getCpeUptimeAvg().get()/record.getCpeUptimeAvg().getDividend():null;
	}

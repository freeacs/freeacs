...
/**
 * A static helper class for parsing the incoming request parameters and translating them to the correct type.
 * 
...
public class InputDataRetriever {
	...
	/**
	 * This method iterates on the methods of the InputData instance reflectively
	 * and tries to retrieve the request parameter value for each Input those methods return.
	 *
	 ...
	 */
	public static InputData parseInto(InputData inputData, ParameterParser params) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		Method[] methods = inputData.getClass().getMethods();
		for (Method m : methods) {
			if (!Modifier.isStatic(m.getModifiers()) && m.getReturnType() == Input.class && Modifier.isPublic(m.getModifiers())) {

...
public class TriggerHandler {
	...
	public void deleteTrigger(TriggerData inputData) throws SQLException, NoAvailableConnectionException {
		...
		triggers.deleteTrigger(trigger, xaps);
	}
	...
	public void setUnittype(TriggerData inputData) {
		if (inputData.getUnittype().getString() != null && xaps != null) {
	...
	private boolean saveTrigger(Trigger trigger) {
		...
		try {
			unittype.getTriggers().addOrChangeTrigger(trigger, xaps);
	...
	private void fillTrigger(Trigger trigger, TriggerData inputData) {
		if (inputData != null) {
			...
			trigger.setParent(unittype.getTriggers().getById(nullsafeInteger(inputData.getParentTrigger())));
	...
	public DropDownSingleSelect<SyslogEvent> getSyslogEventDropdown(SyslogEvent selectedSyslogEvent) {
		if (getUnittype() == null)
			return null;
		List<SyslogEvent> syslogEventList = Arrays.asList(getUnittype().getSyslogEvents().getSyslogEvents());
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getSyslogEventId(), selectedSyslogEvent, syslogEventList);
	}
	...
	public DropDownSingleSelect<File> getScriptFileDropdown(File selectedFile) {
		...
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getScriptFileId(), selectedFile, fileList);
	}

	public List<TableElement> getTriggerTableElements() {
		try {
			return new TableElementMaker().getTriggers(unittype, xaps);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public DropDownSingleSelect<Trigger> getTriggerParentDropdown(Trigger trigger) {
		...
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getParentTrigger(), parentTrigger, triggerList);
	}

	private ArrayList<Trigger> createFilteredList(Trigger selectedTrigger) {
		ArrayList<Trigger> allTriggersList = new ArrayList<Trigger>(Arrays.asList(getUnittype().getTriggers().getTriggers()));
		Iterator<Trigger> i = allTriggersList.iterator();
		while (i.hasNext()) {
			if (i.next().getTriggerType() == Trigger.TRIGGER_TYPE_BASIC)
				i.remove();
		}
	...
	public DropDownSingleSelect<NotifyType> getNotifyTypeDropdown(Integer selectedActionType) {
		...
		NotifyType selectedNotify = selectedActionType == null ? triggerNotifyTypes.get(0) : triggerNotifyTypes.get(selectedActionType);
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getNotifyType(), selectedNotify, triggerNotifyTypes);
	}
	...
	public DropDownSingleSelect<TriggerType> getTriggertypeDropdown(Integer selectedTriggertypeId) {
		...
		TriggerType selectedTriggerType = selectedTriggertypeId == null ? triggerTypes.get(0) : triggerTypes.get(selectedTriggertypeId);
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getTriggerType(), selectedTriggerType, triggerTypes);
	}
	...
	public DropDownSingleSelect<String> getNewEvalPeriodDropdown(Integer selectedEvalPeriod) {
		...
		int selectedIndex = selectedEvalPeriod == null ? 0 : periods.indexOf(String.valueOf(selectedEvalPeriod));
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getEvalPeriodMinutes(), periods.get(selectedIndex), periods);
	}

	public DropDownSingleSelect<NotifyIntervalHour> getNotifyIntervalHoursDropdown(Integer selectedNotifyIntervalHourId) {
		...
		NotifyIntervalHour selectedHour = getSelectedNotifyIntervalHour(intervals, selectedNotifyIntervalHourId);
		return InputSelectionFactory.getDropDownSingleSelect(inputData.getNotifyIntervalHours(), selectedHour, intervals);
	}

	private NotifyIntervalHour getSelectedNotifyIntervalHour(List<NotifyIntervalHour> intervals, Integer selectedNotifyIntervalHourId) {

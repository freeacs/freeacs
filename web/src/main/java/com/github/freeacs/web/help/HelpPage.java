...
/**
 * <p>Is basically used to provide in-page help for pages in xAPS Web,<br />
 * but can possibly be used to describe any topic within the xAPS system.</p>
 * 
...
public abstract class HelpPage {
	 ...
	 * 
	 * Wraps it in a help page.
	 * 
	 * Automatically including:
	 * - SomePage.js
	 * - SomePage.css
	 * into the main template in &lt;script&gt; and &lt;style&gt; tags within the header.
	 *
	 * @param page the page
	...
	public static String getHTMLForPageByString(String page) throws IOException,TemplateException {
		...
		String html = getFileContents("pages/"+shortName.toLowerCase() + "/" + className + ".html",NO_DATA);
		
		String contents = getBodyContents(html);
		map.put("content", contents);
		
		String javascript = getFileContents("pages/"+shortName.toLowerCase() + "/" + className + ".js","");
	...
	public static String getHTMLForPage(Page page) throws IOException,TemplateException {
		...
		if(is!=null){
			...
			char[] buf = new char[is.available()]; br.read(buf);
	...
	/**
	 * Getting the contents within a BODY in HTML.
	 * If not present, returns the original html.
	 *
	 * @param html the html
	 ...
	 * Retrieves the content from an HTML file.
	 * Wraps it in a help page.
	 * Does not include any other files.
	 *
	 ...
	 * @throws TemplateException the template exception
	 */
	public static String getHTMLForPageByResourcePath(String path) throws IOException,TemplateException {
		Template template = config.getTemplate("HelpPage.ftl");

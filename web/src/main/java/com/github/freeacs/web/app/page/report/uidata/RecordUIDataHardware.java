...
/**
 * This is a wrapper class for RecordHardware. Calculates percents and human
 * friendly information.
 * 
 * Contains a static converter method
 * <code>convertRecords(List<RecordHardware> records)</code> for converting a
 * larger set of RecordHardware objects.
 * 
...
public class RecordUIDataHardware extends RecordHardware {
	...
	RecordUIDataHardware(RecordHardware record, Unit unit, RecordUIDataHardwareFilter limits) {
		super(record.getTms(), record.getPeriodType(),
				record.getUnittypeName(), record.getProfileName(), record
						.getSoftwareVersion());
		add(record);
		this.unit = unit;
		this.limits = limits;
		bootTotal = getBootCount().get() / getBootCount().getDividend();
		bootPower = getBootPowerCount().get()
				/ getBootPowerCount().getDividend();
		...
		if (getMemoryHeapDdrCurrentAvg().get() != null)
			memoryHeapDdrCurrentAvg = getMemoryHeapDdrCurrentAvg().get()
					/ getMemoryHeapDdrCurrentAvg().getDividend();
		if (getMemoryHeapDdrLowAvg().get() != null)
			memoryHeapDdrLowestAvg = getMemoryHeapDdrLowAvg().get()
					/ getMemoryHeapDdrLowAvg().getDividend();
		if (getMemoryHeapDdrPoolAvg().get() != null)
			memoryHeapDdrPoolAvg = getMemoryHeapDdrPoolAvg().get()
					/ getMemoryHeapDdrPoolAvg().getDividend();
		if (getMemoryHeapOcmCurrentAvg().get() != null)
			memoryHeapOcmCurrentAvg = getMemoryHeapOcmCurrentAvg().get()
					/ getMemoryHeapOcmCurrentAvg().getDividend();
		if (getMemoryHeapOcmLowAvg().get() != null)
			memoryHeapOcmLowestAvg = getMemoryHeapOcmLowAvg().get()
					/ getMemoryHeapOcmLowAvg().getDividend();
		if (getMemoryHeapOcmPoolAvg().get() != null)
			memoryHeapOcmPoolAvg = getMemoryHeapOcmPoolAvg().get()
					/ getMemoryHeapOcmPoolAvg().getDividend();
		if (getCpeUptimeAvg().get() != null)
			upTimeAvg = getCpeUptimeAvg().get()
					/ getCpeUptimeAvg().getDividend();
		memoryHeapDdrUsagePercent = getPercentageUsed(memoryHeapDdrCurrentAvg,
				memoryHeapDdrPoolAvg);
		memoryHeapOcmUsagePercent = getPercentageUsed(memoryHeapOcmCurrentAvg,
				memoryHeapOcmPoolAvg);
		try {
			String score = "100";
			if (getBootMessage() != null)
				score = "70";
			rowBackgroundStyle = new AbstractWebPage.RowBackgroundColorMethod()
					.exec(Arrays.asList(score));
	...
	public static String getTableOfMemoryUnused(List<RecordUIDataHardware> records){
		...
		for(RecordUIDataHardware record:records){
			html+="<tr><td border=1>"+record.getTmsAsString()+"<br />Memory used:<br />Heap(Ocm: "+record.getMemoryHeapOcmUsagePercentAsString()+"%, Ddr: "+record.getMemoryHeapDdrUsagePercentAsString()+"%)</td></tr>";
		}
		html+="</table>";
		return html;
	}
	...
	/**
	 * First calculate the percent of currently available memory Then subtract
	 * that value from 100 to get the percentage of used memory.
	 * 
	 * @param current
	 *            the current
	 * @param pool
	 ...
	 * @return the list
	 */
	public static List<RecordUIDataHardware> convertRecords(Unit unit,
			List<RecordHardware> records, RecordUIDataHardwareFilter limits) {
	...
	public String getBootMessage() {
		...
		if (super.getBootWatchdogCount().get() != null
				&& super.getBootWatchdogCount().get() > 0)
			string.append("Watchdog boot,");
		if (super.getBootPowerCount().get() != null
				&& super.getBootPowerCount().get() > 0)
			string.append("Power boot,");
		if (super.getBootMiscCount().get() != null
				&& super.getBootMiscCount().get() > 0)
			string.append("Miscellaneous boot,");
		if (super.getBootProvBootCount().get() != null
				&& super.getBootProvBootCount().get() > 0)
			string.append("Provisioning boot,");
		if (super.getBootProvConfCount().get() != null
				&& super.getBootProvConfCount().get() > 0)
			string.append("Provisioning config boot,");
		if (super.getBootProvCount().get() != null
				&& super.getBootProvCount().get() > 0)
			string.append("Provisioning boot,");
		if (super.getBootProvSwCount().get() != null
				&& super.getBootProvSwCount().get() > 0)
			string.append("Provisioning software boot,");
		if (super.getBootResetCount().get() != null
				&& super.getBootResetCount().get() > 0)
			string.append("Reset boot,");
		if (super.getBootUserCount().get() != null
				&& super.getBootUserCount().get() > 0)
	...
	public String getMemoryHeapDdrCurrentAvgAsString() {
		return RecordUIDataMethods
				.getMegaBytePresentation(memoryHeapDdrCurrentAvg);
	}
	...
	public String getMemoryHeapOcmCurrentAvgAsString() {
		return RecordUIDataMethods
				.getMegaBytePresentation(memoryHeapOcmCurrentAvg);
	}
	...
	public String getMemoryHeapDdrLowestAvgAsString() {
		return RecordUIDataMethods
				.getMegaBytePresentation(memoryHeapDdrLowestAvg);
	}
	...
	public String getMemoryHeapOcmLowestAvgAsString() {
		return RecordUIDataMethods
				.getMegaBytePresentation(memoryHeapOcmLowestAvg);
	}
	...
	public String getUpTimeAvgAsString() {
		String upTimeString = RecordUIDataMethods
				.getToStringOrNonBreakingSpace(upTimeAvg);
		return RecordUIDataMethods.appendStringIfNotNonBreaking(upTimeString,
				"m");
	}
	...
	public String getMemoryHeapDdrPoolAvgAsString() {
		return RecordUIDataMethods
				.getMegaBytePresentation(memoryHeapDdrPoolAvg);
	}
	...
	public String getMemoryHeapOcmPoolAvgAsString() {
		return RecordUIDataMethods
				.getMegaBytePresentation(memoryHeapOcmPoolAvg);
	}

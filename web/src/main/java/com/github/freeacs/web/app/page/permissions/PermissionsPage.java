...
public class PermissionsPage extends AbstractWebPage {
	...
	public void process(ParameterParser req, Output outputHandler, DataSource xapsDataSource, DataSource syslogDataSource) throws Exception {
		...
		if (button != null) {
			if (button.equals("Update user") && user != null) {
				...
				if (configure != null && configure.equals("true"))
				...
				if (username != null && fullname != null && password != null && modules != null) {
					...
					if ((userExists = users.getUnprotected(username)) == null || username.equals("admin")) {
						...
						if (newUser.getId() != null) {
							req.getHttpServletRequest().getSession().setAttribute("message", "Successfully added user \"" + username + "\"");
	 ...
	 */
	private Permission[] getTemporaryPermissions() throws IllegalArgumentException, SecurityException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {

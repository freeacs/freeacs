 ...
 * 
 * <ol>
 * <li>If Input has an incoming value from request, then the SessionData object is updated with this value.</li>
 * <li>If Input has no incoming value, the Input's value is set to the value of that from the SessionData object.</li>
 * </ol>
 * 
 * In earlier times this class also checked the session data integrity, 
 * eg if the unittype, profile and unit was related and valid.
 * This has been scrapped to avoid high loading time.
 * 
...
public class InputDataIntegrity {
	...
	/**
	 * In addition to calling the same method with a few different parameters,
	 * this method also adds a context item to the responsehandler, also called a trail point.
	 * 
	...
	/**
	 * Takes a sessionId and a variable number of Input's,
	 * and keeps the Input's value in sync with the SessionData object.
	 *
	...
	public static void rememberAndCheck(String sessionId, Input... inputs) throws SQLException {
		...
		if (unittypeInput != null && unittypeInput.getValue() == null) {
			...
			if (unittypeInput.getValue() != null && !unittypeInput.getValue().equals(sessionData.getUnittypeName()))
		...
		if (profileInput != null && profileInput.getValue() == null) {
			if (!unittypeChanged && sessionData.getUnittypeName() != null && sessionData.getProfileName() != null) {
		...
		if (unitInput != null && unitInput.getValue() == null) {
			if (sessionData.getUnitId() != null && sessionData.getProfileName() != null && sessionData.getUnittypeName() != null) {

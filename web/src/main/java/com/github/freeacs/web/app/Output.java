...
public class Output {
	...
	/**
	 * Deliver the response. To avoid thread problems we synchronize the use of this object within this method.
	 * 
		...
		synchronized(this){
			...
			try{
				...
				if (getDirectResponseString()!=null) {
					printPartialPage(getDirectResponseString(), currentPage,inputParameters, servletResponseChannel);
					return;
				}
				
			    populateTemplateMapWithContextBar();
				
				// If async parameter is true, only parse the page template, and then deliver the partial response
	...
	private void deliverPartially() throws TemplateException, IOException, ServletException {
		...
		String content = Freemarker.parseTemplate(templateMap, freemarkerConfig.getTemplate(getTemplatePath()));
	...
	private void processContextBarUpdates() throws IOException, SQLException {
		...
		if(cU!=null){
			...
			ACSUnit xaps = XAPSLoader.getXAPSUnit(inputParameters.getSession().getId(), xapsDataSource, syslogDataSource);
			Unit unit = null;
	...
	public void addNoCacheToResponse() {
		...
		servletResponseChannel.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, post-check=0, pre-check=0");
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static void writeImageBytesToResponse(byte[] image,HttpServletResponse servletResponseChannel) throws IOException{
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static void writeBytesToResponse(byte[] image,HttpServletResponse servletResponseChannel) throws IOException {
	 ...
	 */
	  ...
	  * @throws IOException the redirect failed
	  */
 	public void redirect(String url,ParameterParser params,HttpServletResponse res) throws IOException {
 		if (url != null) {
	 ...
	 * @throws TemplateException the template exception
	 */
	private void printPartialPage(String pageContents,WebPage currentPage,ParameterParser params,HttpServletResponse res) throws IOException, ServletException, TemplateException {
	 ...
	 * @throws ServletException the servlet exception
	 */
	private void deliverHTML(String outputString,ParameterParser params,HttpServletResponse res) throws IOException, ServletException {
	...
	/**
	 * Used to set template paths that are using the default template path, the index.ftl file.
	 * 
	 * <br />
	 * 
	 * Example wise: by calling setTemplatePathWithIndex("/report");
	 * 
	 * <br />
	 * 
	...
	/**
	 * Checks if <code>setTemplatePath</code> or <code>setTemplatePathWithIndex</code> has been run for this response handler instance.
	 *

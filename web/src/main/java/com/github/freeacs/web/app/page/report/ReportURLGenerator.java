...
final class ReportURLGenerator extends org.jfree.chart.urls.CustomXYURLGenerator {
	 ...
	 * The constructor.
	 * 
	 * Initializes and updates the url format based on the aggregation keys given.
	 * 
	 ...
	 * @return an object array where the two first items is series and item, and the latter items is calculated.
	 */
	private Object[] getObjects(int series, int item) {
		...
		// If so we have an aggregated diagram that needs the object array we get by splitting the series key added to the returned object array.
		...
		if(!name.equals("Total") && !name.contains("|"))

 ...
 * 
 * Contains a static converter method <code>convertRecords(List<RecordVoip> records)</code> for converting a larger set of RecordVoip objects.
 * 
...
public class RecordUIDataVoip extends RecordVoip {
	...
	public RecordUIDataVoip(RecordVoip record){
		super(record.getTms(),record.getPeriodType(),record.getUnittypeName(),record.getProfileName(), record.getSoftwareVersion(), record.getLine());
		add(record);
		...
		if(getPercentLossAvg().get()!=null)
			percentLossAvg = (double)getPercentLossAvg().get()/(double)getPercentLossAvg().getDividend();
		...
		try {
			if(totalScore!=null)
				rowBackgroundStyle = new AbstractWebPage.RowBackgroundColorMethod().exec(Arrays.asList(totalScore.toString()));

...
public class BackgroundProcesses {
  ...
  public static void initiate(DBI dbi) {
    ...
    scheduler.registerTask(
        new Schedule(10000, false, ScheduleType.INTERVAL, new StabilityTask("StabilityLogger")));
    scheduler.registerTask(
        new Schedule(
            5000, false, ScheduleType.INTERVAL, new MessageListenerTask("MessageListener", dbi)));
    scheduler.registerTask(
        new Schedule(
            1000, false, ScheduleType.INTERVAL, new ScheduledKickTask("ScheduledKick", dbi)));
    scheduler.registerTask(
        new Schedule(
            5 * 60000,
            false,

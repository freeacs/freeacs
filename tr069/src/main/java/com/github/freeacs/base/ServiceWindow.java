...
public class ServiceWindow {
	...
	public ServiceWindow(SessionDataI sessionData, boolean disruptive) {
		...
		if (disruptive)
			timeWindow = new TimeWindow(ACSParameters.getValue(SystemParameters.SERVICE_WINDOW_DISRUPTIVE));
		else
	...
	public boolean isWithin(long tms) {
		if (isEnabled())
			return timeWindow.isWithinTimeWindow(tms);
		else
			return true;
	}
	 ...
	 * 
	 * CPE contacts server at 0801 (first time after job has been created).
	 * The job executes at 0801, 0821, 0841 on Monday. Then it continues to
	 * execute at 0801, 0821 and 0841 on Tuesday. All the in-between
	 * job executions are skipped.
	 * 
	...
	public long calculateNextRepeatableTms(Long lastRunTms, long fixedInterval) {
		...
		while (true) {
			...
			if (timeToNRT < -PIIDecision.MINIMUM_PII * 1000l) { // do not test old tms, but let tms close to current time pass
				Log.debug(ServiceWindow.class, "Suggested NRT was rejected as too old (timeToNRT = " + timeToNRT / 1000 + " sec)");
				continue;
			}
	...
	/**
	 * Calculates the PeriodicInformInterval (in seconds) in standard way 
	 * following these rules or guidelines:
	 ...
	 *  d) PII can be minimum 31 seconds
	 * If Not Enabled, it will be calculated according to b) and d) only
	 * @return
	 */
	public long calculateStdPII() {
		if (isEnabled()) {
			...
			if (timeWindow.isWithinTimeWindow(currentTms)) {
				long leftOfTW = timeWindow.getPreviousStartTms(currentTms) + timeWindow.getDailyLength() - currentTms;
				if (nextInterval > leftOfTW)
					nextInterval -= leftOfTW; // More calculation will be necessary
				else
					nextPIITms = currentTms + nextInterval; // We have our final time stamp..
			}

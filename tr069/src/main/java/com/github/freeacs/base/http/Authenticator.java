...
public class Authenticator {
  ...
  public static boolean authenticate(HTTPReqResData reqRes, Properties properties) throws TR069AuthenticationException {
    ...
    try {
      if (properties.isDiscoveryMode() || "basic".equalsIgnoreCase(auth_method)) {
        authenticated = BasicAuthenticator.authenticate(reqRes, properties.isDiscoveryMode(),
            properties.getDiscoveryBlock());
      } else if ("digest".equalsIgnoreCase(auth_method)) {
        authenticated = DigestAuthenticator.authenticate(reqRes, properties.isDiscoveryMode(),
            properties.getDigestSecret());

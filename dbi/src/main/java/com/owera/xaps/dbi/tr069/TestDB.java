...
public class TestDB {
		...
		try {
			...
			while (rs.next()) {
				...
				if (tc == null) {
					...
					tc = new TestCase(unittype, TestCaseMethod.valueOf(rs.getString("tc.method")), tags, expectError);
					tc.setId(tcId);
					map.put(tcId, tc);
				}
	...
	public List<TestCase> getCompleteTestCases(Unittype unittype,
			TestCaseMethod method, String paramFilter, String tagFilter)
...
//			ds.addSqlAndArguments("AND tc.unit_type_id = ? ", unittype.getId());
			...
			while (rs.next()) {
				...
				if (tc == null) {
					...
					if (expectErrorStr != null)
						expectError = expectErrorStr.equals("1") ? true : false;
					tc = new TestCase(unittype, TestCaseMethod.valueOf(rs

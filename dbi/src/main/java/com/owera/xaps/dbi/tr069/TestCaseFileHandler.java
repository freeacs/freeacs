	...
	public TestCase parseFile(String filename) throws IOException, NoAvailableConnectionException, SQLException {
		...
		while ((line = br.readLine()) != null) {
			...
			if (line.toUpperCase().startsWith("ID") && line.length() > 2)
				...
				tc.setMethod(TestCaseMethod.valueOf(array[1].toUpperCase()));
				if (tc.getMethod() == TestCaseMethod.FILE) {
				...
				com.owera.xaps.dbi.File f = unittype.getFiles().getByName(array[1]);
				if (f == null) {
					...
					fs.read(b);
					fs.close();
					f = new com.owera.xaps.dbi.File(unittype, array[1], FileType.MISC,
							null, "1", null, null, unittype.getXaps().getUser());
					f.setBytes(b);
				}
				tc.getFiles().setInputFile(f);
				unittype.getFiles().addOrChangeFile(f, unittype.getXaps());
			}
			...
			if (line.toUpperCase().startsWith(TestCaseParameterType.SET.toString()) && line.length() > 3) {
				...
				if (tc.getMethod() == TestCaseMethod.VALUE)
		...
		if (tc.getMethod() == TestCaseMethod.FILE && tc.getFiles().getInputFile() == null) {

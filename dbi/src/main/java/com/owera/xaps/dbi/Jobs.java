...
public class Jobs {
	...
	private void updateMandatoryJobParameters(Job job, XAPS xaps) throws SQLException, NoAvailableConnectionException {
		if (job.getFlags().getType() == JobType.SOFTWARE) {
			...
			UnittypeParameter jobUtp = SystemParameters.getTR069ScriptParameter(job.getFile().getTargetName(), TR069ScriptType.Version, xaps, job.getUnittype());
	...
	//	private boolean isDependencyEqualSoftwareVersion(Job job) {
	//		if (job.getDependency() == null)
	//			return true;
	//		if (job.getDependency().getSoftware() == null && job.getSoftware() == null)
	//			return true;
	//		if (job.getDependency().getSoftware().getId().intValue() == job.getSoftware().getId().intValue())
	//			return true;
	//		return false;
	//	}

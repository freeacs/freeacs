...
public class UnitQueryWithinUnittype {
	 ...
	 */
	public static boolean match(String fixedOp, String varOp, Operator op, ParameterDataType type) {
		// If fixedOp == null, varOp can match only if null and operator is EQ, or the opposite.
		if (fixedOp == null) {
			if (op.equals(Operator.EQ) && varOp == null)
				return true;
			else if (op.equals(Operator.EQ) && varOp != null)
				return false;
			else if (op.equals(Operator.NE) && varOp == null)
				return false;
			else if (op.equals(Operator.NE) && varOp != null)
				return true;
		...
		if (varOp == null) {
			if (op.equals(Operator.NE) && fixedOp != null)
				return true;
			return false; // can hit here if operator is not EQ nor NE			
		}
		// Neither varOp nor fixedOp can be NULL here.
		if (type.equals(ParameterDataType.NUMBER)) {
			try {
				Long operandL = new Long(varOp);
				Long ppL = new Long(fixedOp);
				if (op.equals(Operator.EQ) && ppL.longValue() == operandL.longValue())
					return true;
				if (op.equals(Operator.NE) && ppL.longValue() != operandL.longValue())
					return true;
				if (op.equals(Operator.LT) && ppL.longValue() < operandL.longValue())
					return true;
				if (op.equals(Operator.LE) && ppL.longValue() <= operandL.longValue())
					return true;
				if (op.equals(Operator.GE) && ppL.longValue() >= operandL.longValue())
					return true;
				if (op.equals(Operator.GT) && ppL.longValue() > operandL.longValue())
					return true;
				return false;
			} catch (NumberFormatException nfe) {
				return false;
			}
		} else if (type.equals(ParameterDataType.TEXT)) {
			if (op.equals(Operator.EQ)) {
				...
				if (op.equals(Operator.LT) && compareInt < 0)
					return true;
				if (op.equals(Operator.LE) && compareInt <= 0)
					return true;
				if (op.equals(Operator.GE) && compareInt >= 0)
					return true;
				if (op.equals(Operator.GT) && compareInt > 0)
					return true;

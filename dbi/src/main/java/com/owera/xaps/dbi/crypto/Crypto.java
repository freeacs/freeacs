...
public class Crypto {
	...
	public static void main(String[] args) throws Exception {
		CertificateDetails cert1 = new CertificateDetails(Certificate.CERT_TYPE_REPORT, null, null, null);
	...
	public static String computeCertificateFromObject(CertificateDetails cert) throws Exception {
		try {
			String s = null;
			if (cert.getType().equals(Certificate.CERT_TYPE_REPORT))
				s = Certificate.CERT_TYPE_REPORT;
			if (cert.getType().equals(Certificate.CERT_TYPE_PROVISIONING))
				s = Certificate.CERT_TYPE_PROVISIONING;
			if (cert.getCustomerName() != null) {
				s += ", issued to " + cert.getCustomerName();
			} else if (cert.getLimitType() != null) {
				if (cert.getLimitType().equals(Certificate.TRIAL_TYPE_DAYS)) {
					Calendar c = Calendar.getInstance();
					c.add(Calendar.DAY_OF_YEAR, cert.getLimit());
					s += " until " + Certificate.dateFormat.format(c.getTime());

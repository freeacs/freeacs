...
public interface UnittypeDao {
    ...
    @RegisterBeanMapper(UnittypeVO.class)
    List<UnittypeVO> get();
    ...
    @RegisterBeanMapper(UnittypeVO.class)
    Optional<UnittypeVO> get(Long id);

    @SqlUpdate("delete from unit_type where unit_type_id = :id")
    boolean delete(Long id);

    @SqlUpdate("update unit_type set unit_type_name = :unitTypeName, matcher_id = :matcherId, vendor_name = :vendorName, description = :description, protocol = :protocol where unit_type_id = :id")
    boolean update(@BindBean UnittypeVO object);

    @SqlUpdate("insert into unit_type(unit_type_name, matcher_id, vendor_name, description, protocol) values(:unitTypeName, :vendorName, :description, :protocol)")
    @GetGeneratedKeys

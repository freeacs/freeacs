...
public class Crypto {
  private static final BigInteger mPub =
      new BigInteger(
  ...
  private static String decryptChunkUsingRSAPublicKey(String encryptedHex) {
    try {
      ...
      throw new RuntimeException(
          "encrypted string has been tampered with - not possible to decrypt");
    } catch (Throwable t) {
      throw new RuntimeException(
          "An error occured while trying to decrypt (may be wrong version of JVM?) " + encryptedHex,
          t);
    }
  }
      ...
      if (cert.getCustomerName() != null) {
        ...
        if (cert.getLimitType().equals(com.github.freeacs.dbi.Certificate.TRIAL_TYPE_DAYS)) {
          ...
          throw new IllegalArgumentException(
              "Usage: Crypto Report|Provisioning [time <numberOfDays>] [count <maximumCount>]. Supplied: "
                  + cert.toString());
        }
      }
  ...
  private static String computeRSACertificateOfChunk(String s) throws Exception {
    ...
    while ((line = br.readLine()) != null) {
      if (mPriv == null) mPriv = new BigInteger(line);
      else ePriv = new BigInteger(line);
    }
    KeyFactory fact = KeyFactory.getInstance("RSA");

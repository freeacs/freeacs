...
public class Permissions {
  ...
  //	public static final String[] WEB_PAGES = { "search", "unit", "profile", "unittype", "group",
  // "job", "software", "syslog", "report", "monitor", "staging" };
  ...
  public Permission[] getPermissions() {
    ...
    if (permissions.length > 0) {
      for (Set<Integer> permSet : unittypeIdMap.values()) {
        for (Integer permId : permSet) permissions[counter++] = idMap.get(permId);
      }
    }
    return permissions;
  }
  ...
  public Permission getByUnittypeProfile(Integer unittypeId, Integer profileId) {
    ...
    if (permissionIdSet == null) return null;

    for (Integer permissionId : permissionIdSet) {
      Permission p = getById(permissionId);
      if (p.getProfileId() == null && profileId == null) return p;
      if (p.getProfileId() == null || profileId == null) continue;
      if (p.getProfileId().intValue() == profileId) return p;
    }
    return null;
  }
  ...
  protected void delete(Permission permission) throws SQLException {
    ...
    if (permissionIdSet != null) permissionIdSet.remove(permission.getId());
  }

  protected void add(Permission permission) {
    idMap.put(permission.getId(), permission);
    Set<Integer> permissionIdSet = unittypeIdMap.get(permission.getUnittypeId());
    if (permissionIdSet == null) permissionIdSet = new TreeSet<Integer>();
  ...
  protected void addOrChange(Permission permission) throws SQLException {
    ...
    try {
      ...
      if (permission.getId() == null) {
        ...
        if (gk.next()) permission.setId(gk.getInt(1));

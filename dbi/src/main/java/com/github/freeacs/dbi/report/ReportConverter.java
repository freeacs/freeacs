...
public class ReportConverter {
	...
	public static Report<RecordSyslog> convertSyslogReport(Report<RecordSyslog> r, PeriodType periodType) {
		...
		for (Entry<Key, RecordSyslog> entry : r.getMap().entrySet()) {
			...
			if (key.getPeriodType().isLongerThan(periodType))
				throw new IllegalArgumentException("Cannot convert a voip record to another voip record because its periodType is longer");
			RecordSyslog record = entry.getValue();
			RecordSyslog tmp = new RecordSyslog(key.getTms(), periodType, record.getUnittypeName(), record.getProfileName(), record.getSeverity(), record.getEventId(), record.getFacility());
	...
	public static Report<RecordProvisioning> convertProvReport(Report<RecordProvisioning> r, PeriodType periodType) {
		...
		for (Entry<Key, RecordProvisioning> entry : r.getMap().entrySet()) {
			...
			RecordProvisioning tmp = new RecordProvisioning(key.getTms(), periodType, record.getUnittypeName(), record.getProfileName(), record.getSoftwareVersion(), record.getOutput());
			Key newKey = tmp.getKey();
	...
	public static Report<RecordVoip> convertVoipReport(Report<RecordVoip> r, PeriodType periodType) {
		...
		for (Entry<Key, RecordVoip> entry : r.getMap().entrySet()) {
			...
			RecordVoip tmp = new RecordVoip(key.getTms(), periodType, record.getUnittypeName(), record.getProfileName(), record.getSoftwareVersion(), record.getLine());
			Key newKey = tmp.getKey();
	...
	public static Report<RecordHardware> convertHardwareReport(Report<RecordHardware> r, PeriodType periodType) {
		...
		for (Entry<Key, RecordHardware> entry : r.getMap().entrySet()) {
			Key key = entry.getKey();
			if (key.getPeriodType().isLongerThan(periodType))
				throw new IllegalArgumentException("Cannot convert periodType of the record since the origianl periodType is longer");
			RecordHardware record = entry.getValue();
			RecordHardware tmp = new RecordHardware(key.getTms(), periodType, record.getUnittypeName(), record.getProfileName(), record.getSoftwareVersion());
			Key newKey = tmp.getKey();

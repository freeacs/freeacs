...
public class Unit {
	...
	/**
	 * This get-method will return the set of both profile and unit-parameters.
	...
	public String getParameterValue(String unittypeParameterName, boolean useProfileIfNecessary) {
		if (!paramsAvailable)
			throw new IllegalArgumentException("Cannot ask for parameter values from unit before loading parameters");
		if (unitParameters != null) {
			UnitParameter up = unitParameters.get(unittypeParameterName);
			if (up != null) {
				if (!up.getParameter().valueWasNull())
					return up.getValue();
				// else
				// return null;
			}
	...
	//		try {
	//			return ProvisioningState.valueOf(getParameterValue(SystemParameters.PROVISIONING_STATE, false));
	//		} catch (Throwable t) {
	...
	public synchronized void toWriteQueue(String unittypeParameterName, String value) {
		if (unittype == null || profile == null)
			throw new IllegalArgumentException("Cannot add to write queue in Unit because Unittype or Profile object is null");
	...
	public synchronized void toDeleteQueue(String unittypeParameterName) {
		if (unittype == null || profile == null)
			throw new IllegalArgumentException("Cannot add to delete queue in Unit because Unittype or Profile object is null");
		UnittypeParameter utp = unittype.getUnittypeParameters().getByName(unittypeParameterName);

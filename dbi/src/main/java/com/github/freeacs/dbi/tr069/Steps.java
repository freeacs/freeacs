...
public class Steps {
      ...
      for (Step alreadyAddedStep : stepArray) {
        if (alreadyAddedStep == step)
          throw new IllegalArgumentException(
              "The step " + stepStr.trim() + "  is invalid because it is repeated");
      }
  ...
  public boolean lastStep() {
    if (currentStep == steps[steps.length - 1]) return true;
    else return false;
  }

  /**
   * Will return GET or FAC: SET: Will reset any state about RESET GET: MUST come after SET, but no
   * RESET can come between SET and GET FAC: MUST come after SET AND a RESET
   *
   * <p>Will return null if GET is first in step-sequence --> no comparison with TestCaseParameter
   *
   * @return
  ...
  public TestCaseParameterType getCompareType() {
    ...
    for (Step s : steps) {
      ...
      if (s == Step.RESET) reset = true;
      if (s == Step.GET) {
        if (setMode && reset) return TestCaseParameterType.FAC;
        if (setMode) return TestCaseParameterType.GET;
      }
    }
    return null;
  }

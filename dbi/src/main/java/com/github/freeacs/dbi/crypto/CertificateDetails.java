...
public class CertificateDetails {
  ...
  private void setType(String type) {
    if (type != null
        && (type.equals(Certificate.CERT_TYPE_PROVISIONING)
            || type.equals(Certificate.CERT_TYPE_REPORT))) this.type = type;
    else
      throw new IllegalArgumentException(
          "Certificate type must be "
              + Certificate.CERT_TYPE_PROVISIONING
              + " or "
              + Certificate.CERT_TYPE_REPORT);
  }
  ...
  private void setLimitType(String limitType) {
    if (limitType != null
        && (limitType.equals(Certificate.TRIAL_TYPE_COUNT)
            || limitType.equals(Certificate.TRIAL_TYPE_DAYS))) this.limitType = limitType;
    else if (limitType != null)
      throw new IllegalArgumentException(
          "Certificate limit type must be "
              + Certificate.TRIAL_TYPE_DAYS
              + " or "
              + Certificate.TRIAL_TYPE_COUNT);
  }

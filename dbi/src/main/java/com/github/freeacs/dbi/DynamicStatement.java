...
public class DynamicStatement {
  ...
  public void addSqlAndStringArgs(String sql, String... strs) {
    this.sql.append(sql);
    for (String s : strs) {
      if (s == null) arguments.add(new NullString());
  ...
  public void addSqlAndIntegerArgs(String sql, Integer... ints) {
    this.sql.append(sql);
    for (Integer i : ints) {
      if (i == null) arguments.add(new NullInteger());
      else arguments.add(i);
    }
  }
  ...
  public PreparedStatement makePreparedStatement(Connection c, String column) throws SQLException {
    if (column != null) return makePreparedStatement(c, new String[] {column});
    else return makePreparedStatement(c, (String[]) null);
  }

  public PreparedStatement makePreparedStatement(Connection c, String[] columns)
      throws SQLException {
    PreparedStatement pp = null;
    if (columns == null || columns.length == 0) pp = c.prepareStatement(sql.toString());
    else pp = c.prepareStatement(sql.toString(), columns);
    for (int i = 0; i < arguments.size(); i++) {
      Object o = arguments.get(i);
      if (o instanceof String) pp.setString(i + 1, (String) o);
      else if (o instanceof Integer) pp.setInt(i + 1, (Integer) o);
      else if (o instanceof Long) pp.setInt(i + 1, ((Long) o).intValue());
      else if (o instanceof Timestamp) pp.setTimestamp(i + 1, (Timestamp) o);
      else if (o instanceof java.util.Date)
        pp.setTimestamp(i + 1, new Timestamp(((java.util.Date) o).getTime()));
      else if (o instanceof Date) pp.setDate(i + 1, (Date) o);
      ...
      else if (o instanceof NullInteger) pp.setNull(i + 1, Types.INTEGER);
      else if (o instanceof NullString) pp.setNull(i + 1, Types.VARCHAR);
      else pp.setString(i + 1, (String) o);
    }
  ...
  private String getArgument(int i) {
    Object o = arguments.get(i);
    if (o instanceof Timestamp) return "'" + tmsFormat.format(((Timestamp) o)) + "'";
    if (o instanceof Date) return "'" + tmsFormat.format(((Date) o)) + "'";
    if (o instanceof java.util.Date) return "'" + tmsFormat.format(((java.util.Date) o)) + "'";
    if (o instanceof Calendar) return "'" + tmsFormat.format(((Calendar) o).getTime()) + "'";
    if (o instanceof Integer) return "" + o;
    if (o instanceof Long) return "" + o;
    else if (o instanceof Pattern) return "'" + ((Pattern) o).pattern() + "'";
    else if (o instanceof String) {
      String arg = (String) o;
      if (arg.equalsIgnoreCase("SYSDATE")) return arg;
      if (arg.equalsIgnoreCase("SYSTIMESTAMP")) return arg;
      if (arg.equalsIgnoreCase("NOW()")) return arg;
  ...
  public String getSqlQuestionMarksSubstituted() {
    String sql = getSql();
    for (int i = 0; i < arguments.size(); i++) sql = sql.replaceFirst("\\?", getArgument(i));
    return sql;
  }

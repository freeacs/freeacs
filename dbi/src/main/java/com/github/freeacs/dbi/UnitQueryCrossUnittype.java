...
class UnitQueryCrossUnittype {
  ...
  public UnitQueryCrossUnittype(
    ...
    this.connection = c;
    this.acs = acs;
    if (unittypes != null) this.unittypes = unittypes;
    else this.unittypes = new ArrayList<Unittype>();
    if (profiles != null) this.profiles = profiles;
    else this.profiles = new ArrayList<Profile>();
  ...
  public UnitQueryCrossUnittype(Connection c, ACS acs, Unittype unittype, List<Profile> profiles) {
    ...
    if (unittype != null) unittypes.add(unittype);
    if (profiles != null) this.profiles = profiles;
    else this.profiles = new ArrayList<Profile>();
    prepareUnittypesAndProfiles();
  }
  ...
  private void prepareUnittypesAndProfiles() {
    ...
    if (profiles.size() > 0) {
      ...
      if (allProfilesSpecified) {
        ...
        unittypes =
            new ArrayList<Unittype>(); // do not search for unittype, profiles must be specified
      }
    }
    if (user.isAdmin()) {
  ...
  private Map<String, Unit> getUnitsImpl(Map<String, Unit> units, String searchStr, Integer limit)
    ...
    try {
      ...
      rs = pp.executeQuery();
      if (logger.isDebugEnabled()) logger.debug("-SQL: " + ds.getSqlQuestionMarksSubstituted());
      while (rs.next()) {
        ...
        if (unittype == null) break; // can happen if user has no unittypes allowed
  ...
  private DynamicStatement computeSQL(String searchStr) {
    ...
    if (searchStr != null) {
      ...
      if (searchStr.startsWith("%^")) searchStr = searchStr.substring(2);
      else if (searchStr.startsWith("^")) searchStr = searchStr.substring(1);
      if (searchStr.endsWith("$%")) searchStr = searchStr.substring(0, searchStr.length() - 2);
      else if (searchStr.endsWith("$")) searchStr = searchStr.substring(0, searchStr.length() - 1);
      boolean likeness =
          (searchStr.indexOf("%") > -1)
    ...
    if (profiles.size() > 0) ds = searchAmongManyProfiles("u1", ds);
    else if (unittypes.size() > 0) ds = searchAmongManyUnittypes("u1", ds);
    ds.cleanupSQLTail();
    return ds;
  }
   ...
   *
   * <p>If one search for likness, then one must use LIKE instead of =
   *
  ...
  public List<Unit> getUnitsById(List<Unit> units) throws SQLException {
    if (units == null || units.size() == 0) return new ArrayList<Unit>();
    DynamicStatement ds = new DynamicStatement();
    ds.addSql(
    ...
    for (Unit unit : units) {
      if (unit != units.get(units.size() - 1)) sb.append("'" + unit.getId() + "', ");
      else sb.append("'" + unit.getId() + "'");
    }
    ds.addSql("u.unit_id IN (" + sb.toString() + ") AND ");
    if (profiles.size() > 0) ds = searchAmongManyProfiles("u", ds);
    else if (unittypes.size() > 0) ds = searchAmongManyUnittypes("u", ds);
    ds.cleanupSQLTail();
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (lastUnit == null || !lastUnit.getId().equals(uid)) {
          ...
          if (ut != null) pr = ut.getProfiles().getById(profileId);
          unit = new Unit(uid, ut, pr);
          unitsWithDetails.add(unit);
          lastUnit = unit;
        }
        if (!uid.equals(unit.getId())) break; // could happen for unitParamValue-search
  ...
  public Unit getUnitById(String unitId) throws SQLException {
    ...
    ds.addSql(
        "SELECT u.unit_id, u.profile_id, u.unit_type_id, up.unit_type_param_id, up.value FROM unit u ");
    ds.addSql("LEFT JOIN unit_param up ON u.unit_id = up.unit_id WHERE ");
    if (unitId != null) ds.addSqlAndArguments("u.unit_id = ? AND ", unitId);
    if (profiles.size() > 0) ds = searchAmongManyProfiles("u", ds);
    else if (unittypes.size() > 0) ds = searchAmongManyUnittypes("u", ds);
    ds.cleanupSQLTail();
    ResultSet rs = null;
  ...
  /**
   * Retrieves a list of Unit-object. NB!! Only unitId is populated!! This method is meant to be
   * used in conjunction with get getUnitsById(List<Unit>)
   *
  ...
  public List<Unit> getLimitedUnitsByValue(List<String> uniqueParamValues) throws SQLException {
    ...
    for (String uniqueParamValue : uniqueParamValues) {
      ...
      else sb.append("'" + uniqueParamValue + "'");
    }
    DynamicStatement ds = new DynamicStatement();

...
public class TestGenerator {
  ...
  // A structure to keep a series of TR069Parameters in a list, each belonging to the same object,
  // but on different "level/layers" in the
  // TR-069 datamodel
  // Key: Number of layers, Value: Map: Key: ParameterName, Value: TR069Parameter
  private Map<Integer, Map<String, List<UnittypeParameter>>> layerMap =
      new HashMap<Integer, Map<String, List<UnittypeParameter>>>();
  ...
  // Add parameter to the layerMap data structure, building the datastructure as new parameters are
  // added
  private void addToLayerMap(UnittypeParameter utp) {
    String[] paramNameArray = utp.getName().split("\\.");
    if (paramNameArray.length <= 1) return;
  ...
  public List<TestCase> generateSetTcListFromMasterTc(TestCase masterTc) throws Exception {
    ...
    for (TestCaseParameter tcp : masterTc.getParams()) {
      if (tcp.getType() == TestCaseParameterType.SET) stcParams.add(tcp);
    }
    for (int i = 0; i < stcParams.size(); i++) {
      ...
      TestCase tc =
          new TestCase(
              unittype,
              TestCaseMethod.VALUE,
              masterTc.getTags()
                  + "[SET-"
                  + paramsSet
                  + "] ["
                  + dmp.getDatatype().getXsdType()
                  + "]",
              masterTc.getExpectError());
      for (int j = i; j < stcParams.size(); j++) {
        TestCaseParameter stcParam = stcParams.get(j);
        tc.getParams()
            .add(
                new TestCaseParameter(
                    TestCaseParameterType.SET,
                    stcParam.getUnittypeParameter(),
                    stcParam.getDataModelParameter(),
                    stcParam.getValue(),
                    stcParam.getNotification()));
        tc.getParams()
            .add(
                new TestCaseParameter(
                    TestCaseParameterType.GET,
                    stcParam.getUnittypeParameter(),
                    stcParam.getDataModelParameter(),
                    stcParam.getValue(),
                    stcParam.getNotification()));
  ...
  public List<TestCase> generate(String type) throws Exception {
    List<TestCase> generatedTc = new ArrayList<TestCase>();
    if (type == null || type.equals("VALUE")) generateValueTC(generatedTc);
    if (type == null || type.equals("ATTRIBUTE")) generateAttributeTC(generatedTc);
    return generatedTc;
  }
  ...
  private TestCase generateAttributeTestCase(
      TR069DMParameter dmp, UnittypeParameter utp, int notification) {
    TestCase tc = null;
    String notificationRule = dmp.getNotification();
    if (notificationRule == null) notificationRule = "normal";
    if (notificationRule.equals("forceEnabled") && notification < 2) {
      tc =
          new TestCase(
              unittype,
              TestCaseMethod.ATTRIBUTE,
              "[GENERATED] [RULE-"
                  + notificationRule
                  + "] [NOTIFY-"
                  + notification
              ...
              true);
      tc.getParams()
          .add(new TestCaseParameter(TestCaseParameterType.GET, utp, dmp, null, notification));
      tc.getParams()
          .add(new TestCaseParameter(TestCaseParameterType.SET, utp, dmp, null, notification));
  ...
  private void generateLargeValueTC(List<TestCase> generatedTc) {
    ...
    while (i < unittype.getUnittypeParameters().getUnittypeParameters().length) {
      ...
      largeTC
          .getParams()
          .add(new TestCaseParameter(TestCaseParameterType.GET, utp, dmp, "[STRING]", 0));
  ...
  private List<TestCase> generateValueTC(List<TestCase> generatedTc) throws Exception {
    ...
    for (UnittypeParameter utp : unittype.getUnittypeParameters().getUnittypeParameters()) {
      ...
      if (p.getName().contains("ManagementServer.")) continue;
      ...
      while (matcher.find()) {
        ...
        if (number != 1) continue parameters;
      }
      addToLayerMap(utp);
      ...
      if (!p.isReadOnly()) {
        if (p.getDatatype() == TR069DMType.BOOLEAN) {
          ...
          stringcount++;
        } else if (p.getDatatype() == TR069DMType.STRING
            && p.getName().toUpperCase().endsWith("PASSWORD")) {
          generatedTc.add(generateValueStringTestCase(p, utp));
          stringcount++;
        } else {
  ...
  private TestCase generateValueComplexTemplateTestCase(
      Integer layer, List<UnittypeParameter> utpList) {
    TestCase tc =
        new TestCase(
            unittype,
            TestCaseMethod.VALUE,
            "[GENERATED] [TEMPLATE] [COMPLEX] [LAYER" + layer + "]",
            true);
    for (UnittypeParameter utp : utpList) {
      ...
      if (dmp.isReadOnly()) {
        tc.getParams()
            .add(new TestCaseParameter(TestCaseParameterType.GET, utp, dmp, "[STRING]", 0));
        tc.getParams()
            .add(new TestCaseParameter(TestCaseParameterType.FAC, utp, dmp, "[STRING]", 0));
      } else
        tc.getParams()
            .add(
                new TestCaseParameter(
                    TestCaseParameterType.SET, utp, dmp, "HelloWorld-" + utp.getId(), 0));
    }
    return tc;
  }
  ...
  private TestCase generateValueBooleanTestCase(
      TR069DMParameter dmp, UnittypeParameter utp, boolean state) {
    TestCase tc =
        new TestCase(
            unittype,
            TestCaseMethod.VALUE,
  ...
  private TestCase generateValueStringTestCase(TR069DMParameter dmp, UnittypeParameter utp) {
    TestCase tc =
        new TestCase(
            unittype,
            TestCaseMethod.VALUE,
            "[GENERATED] [READWRITE] [" + dmp.getDatatype().getXsdType() + "]",
            null);
    tc.getParams()
        .add(
            new TestCaseParameter(
    ...
    tc.getParams()
        .add(
            new TestCaseParameter(
                TestCaseParameterType.GET, utp, dmp, "HelloWorld-" + utp.getId(), 0));
    return tc;
  }
  ...
  private TestCase generateValueTemplateTestCase(TR069DMParameter dmp, UnittypeParameter utp) {
    TestCase tc =
        new TestCase(
            unittype,
            TestCaseMethod.VALUE,
            "[GENERATED] [READWRITE] [TEMPLATE] [" + dmp.getDatatype().getXsdType() + "]",
            true);
    tc.getParams()
        .add(
            new TestCaseParameter(

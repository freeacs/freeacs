...
public class TestDB {
  ...
  public TestDB(ACS acs) {
    try {
      if (this.tr069DMMap == null) this.tr069DMMap = TR069DMLoader.load();
    } catch (Exception e) {
  ...
  public void deleteTestCase(TestCase tc) throws SQLException {
    ...
    try {
      ...
      ds.addSqlAndArguments("DELETE FROM test_case_param WHERE case_id = ?", tc.getId());
  ...
  public TestCase addOrChangeTestCase(TestCase tc) throws SQLException {
    ...
    try {
      ...
      InsertOrUpdateStatement ious =
          new InsertOrUpdateStatement("test_case", new Field("id", tc.getId()));
      ...
      if (tc.getExpectError() != null) expectError = tc.getExpectError() ? 1 : 0;
      ...
      for (TestCaseParameter tcp : tc.getParams()) {
        InsertOrUpdateStatement iousParam =
            new InsertOrUpdateStatement("test_case_param", new Field("id", tcp.getId()));
        iousParam.addField(new Field("case_id", tc.getId()));
        iousParam.addField(new Field("type", tcp.getType().toString()));
        iousParam.addField(new Field("unit_type_param_id", tcp.getUnittypeParameter().getId()));
        iousParam.addField(new Field("value", tcp.getValue()));
        iousParam.addField(new Field("notification", tcp.getNotification()));
        ps1 = iousParam.makePreparedStatement(c);
        ps1.executeUpdate();
        gk = ps1.getGeneratedKeys();
        if (gk.next()) tcp.setId(gk.getInt(1));
        gk.close();
        ps1.close();
      }
      ...
      if (tc.getFiles() != null) {
        ious =
            new InsertOrUpdateStatement("test_case_files", new Field("id", tc.getFiles().getId()));
        ious.addField(new Field("case_id", tc.getId()));
        ious.addField(new Field("input_file_id", tc.getFiles().getInputFile().getId()));
        if (tc.getFiles().getOutputFile() != null)
          ious.addField(new Field("output_file_id", tc.getFiles().getOutputFile().getId()));
        ps1 = ious.makePreparedStatement(c);
  ...
  public TestCase getTestCase(Unittype unittype, int testCaseId) throws SQLException {
    ...
    try {
      ...
      ds.addSql(
          "SELECT tc.id, tc.method, tc.tag, tc.expect_error, tcp.type,"
      ...
      while (rs.next()) {
        ...
        if (tc == null) {
          ...
          if (expectErrorStr != null) expectError = expectErrorStr.equals("1") ? true : false;
          tc =
              new TestCase(
                  unittype,
                  TestCase.TestCaseMethod.valueOf(rs.getString("tc.method")),
                  tags,
                  expectError);
          tc.setId(tcId);
          map.put(tcId, tc);
        ...
        if (tcpId != 0) {
          ...
          UnittypeParameter utp =
              unittype.getUnittypeParameters().getById(rs.getInt("tcp.unit_type_param_id"));
      ...
      else return map.get(testCaseId);
    } catch (SQLException sqlex) {
      sqle = sqlex;
      throw sqlex;
    } finally {
  ...
  /**
   * This method is only used by TR-069 server to get a list of complete TestCases - that is test
   * cases with all test case parameters. The method is slow, since it must fire one SQL for each
   ...
   * @throws SQLException
   */
  public List<TestCase> getCompleteTestCases(
      Unittype unittype, TestCase.TestCaseMethod method, String paramFilter, String tagFilter)
      throws SQLException {
    List<TestCase> testCaseList = getUncompleteTestCases(unittype, method, paramFilter, tagFilter);
  ...
  /**
   * This method will not return complete Test Case objects, since not all test case parameters are
   * found. The method is useful to produce a list of test cases available for the given filters.
   *
   * @param unittype
   * @param method
   * @param paramFilter
   ...
   * @throws SQLException
   */
  public List<TestCase> getUncompleteTestCases(
      Unittype unittype, TestCase.TestCaseMethod method, String paramFilter, String tagFilter)
      throws SQLException {
    Connection c = dataSource.getConnection();
    PreparedStatement ps = null;
    ...
    try {
      ...
      //					" ON tc.id = tcf.case_id" +
      //					" WHERE true "); //ugly hack to put the following AND-clauses after a WHERE instead of
      // a ON.
      ...
      if (tagFilter != null) {
        ...
        for (String tag : tags) ds.addSqlAndArguments("AND tc.tag LIKE ? ", "%[" + tag + "]%");
      }
      //			ds.addSqlAndArguments("AND tc.unit_type_id = ? ", unittype.getId());
      if (method != null) ds.addSqlAndArguments("AND tc.method = ? ", method.toString());
  ...
  public void addOrChangeTestHistory(TestHistory history) throws SQLException {
    Connection c = dataSource.getConnection();
    PreparedStatement ps = null;
    SQLException sqle = null;
    ResultSet gk = null;
    try {
      InsertOrUpdateStatement ious =
          new InsertOrUpdateStatement("test_history", new Field("id", history.getId()));
      ious.addField(new Field("unit_type_id", history.getUnittype().getId()));
      ious.addField(new Field("unit_id", history.getUnitId()));
      ious.addField(new Field("case_id", history.getTestCaseId()));
      ious.addField(new Field("start_timestamp", history.getStartTimestamp()));
      if (history.getEndTimestamp() != null)
        ious.addField(new Field("end_timestamp", history.getEndTimestamp()));
      if (history.getFailed() == null) ious.addField(new Field("failed", 0));
      else ious.addField(new Field("failed", history.getFailed()));
      if (history.getExpectError() == null) ious.addField(new Field("expect_error", false));
      else ious.addField(new Field("expect_error", history.getExpectError()));
      String result = history.getResult();
      if (history.getResult() != null && history.getResult().length() > 4096) {
  ...
  public int deleteHistory(TestHistory filter) throws SQLException {
    ...
    try {
      ...
      if (filter.getId() != null) ds.addSqlAndArguments("id = ? AND ", filter.getId());
      if (filter.getUnittype() != null)
        ds.addSqlAndArguments("unit_type_id = ? AND ", filter.getUnittype().getId());
      ...
      if (filter.getStartTimestamp() != null)
        ds.addSqlAndArguments("start_timestamp >= ? AND ", filter.getStartTimestamp());
      if (filter.getEndTimestamp() != null)
        ds.addSqlAndArguments("start_timestamp <= ? AND ", filter.getEndTimestamp());
      if (filter.getTestCaseId() != null)
        ds.addSqlAndArguments("case_id = ? AND ", filter.getTestCaseId());
      if (filter.getUnitId() != null) ds.addSqlAndArguments("unit_id = ? AND ", filter.getUnitId());
      ds.cleanupSQLTail();
      ps = ds.makePreparedStatement(c);
  ...
  public List<TestHistory> getHistory(TestHistory filter) throws SQLException {
    Connection c = dataSource.getConnection();
    PreparedStatement ps = null;
    SQLException sqle = null;
    ResultSet rs = null;
    try {
      ...
      while (rs.next()) {
        ...
        boolean expectError = rs.getInt("expect_error") == 1 ? true : false;
        if (unittype == null) continue; // no access to this unittype with this user
        Date startTimestamp = rs.getTimestamp("start_timestamp");
        TestHistory th = new TestHistory(unittype, startTimestamp, unitId, testCaseId, expectError);

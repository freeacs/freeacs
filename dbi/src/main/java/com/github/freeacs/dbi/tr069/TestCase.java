...
public class TestCase {
  ...
  public String getIdFreeTags() {
    if (tags != null) return tags.replaceAll("[ID-" + id + "]", "");
    else return null;
  }
  ...
  public static List<String> getTagList(String tags) {
    ...
    if (tags.contains("[")) {
      while (true) {
        ...
        if (endPos == -1 || endPos <= startPos + 1) break;
  ...
  public String toString() {
    ...
    sb.append(
        "# The METHOD can be either VALUE, ATTRIBUTE or FILE. Make sure that the parameter definitions below match the method type\n");
    ...
    if (files != null && files.getInputFile() != null) {
      sb.append(
          "# If VALUE is FILE, specifies an input file to send instead of SET/GET/FAC TR-069 parameters.\n");
      sb.append(String.format("%-12s", "INPUT") + files.getInputFile().getName() + "\n\n");
    }

    if (files != null && files.getOutputFile() != null) {
      sb.append(
          "# If VALUE is FILE, specifies an output file to send instead of SET/GET/FAC TR-069 parameters.\n");
      ...
      if (tcp.getType() == TestCaseParameterType.SET) {
        if (method == TestCaseMethod.VALUE)
          sb.append(
              String.format(
                      "%1$-12s%2$-120s",
                      tcp.getType().toString(), tcp.getUnittypeParameter().getName())
                  + " "
                  + tcp.getValue()
                  + "\n");
        else
    ...
    sb.append(
        "# Specify one more GET parameters, to be used in either SetParameterValue or SetParameterAttribute method.\n");
    sb.append(
        "# If METHOD is ATTRIBUTE, a number between 0 and 2 is expected, and will be used as the notification value\n");
    sb.append(
        "# If METHOD is VALUE, a value/string is expected, and will be used as the parameter value\n");
    ...
    sb.append(
        "#\tExample       : GET InternetGatewayDevice.DeviceInfo.ProvisioningCode HelloWorld\n");
    ...
    sb.append(
        "# Specify one more FAC parameters, to be used in either SetParameterValue or SetParameterAttribute method.\n");
    sb.append(
        "# If METHOD is ATTRIBUTE, a number between 0 and 2 is expected, and will be used as the notification value\n");
    sb.append(
        "# If METHOD is VALUE, a value/string is expected, and will be used as the parameter value\n");
    ...
    sb.append(
        "#\tExample       : FAC InternetGatewayDevice.DeviceInfo.ProvisioningCode HelloWorld\n");
  ...
  public boolean equals(Object o) {
    ...
    if (tc.getExpectError() == null && this.getExpectError() != null
        || tc.getExpectError() != null && this.getExpectError() == null) return false;
    if (tc.getExpectError() != null
        && tc.getExpectError().booleanValue() != this.getExpectError().booleanValue()) return false;

    if (tc.getMethod() != this.getMethod()) return false;

    if (tc.getTags() == null && this.getTags() != null
        || tc.getTags() != null && this.getTags() == null) return false;
    if (tc.getIdFreeTags() != null && !tc.getIdFreeTags().equals(this.getIdFreeTags()))
      return false;

    if (tc.getUnittype().getId().intValue() != this.getUnittype().getId()) return false;

    if (tc.getParams().size() != this.getParams().size()) return false;
    ...
    for (TestCaseParameter tcParam : tc.getParams()) {
      for (TestCaseParameter thisParam : this.getParams()) {
        if (tcParam.equals(thisParam)) equalCount++;
      }
    }
    if (equalCount != this.getParams().size()) return false;

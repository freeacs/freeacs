...
public class TestCaseParameter {
  ...
  public TestCaseParameter(
      TestCaseParameterType type,
      UnittypeParameter utp,
      TR069DMParameter dmp,
      String value,
      int notification) {
    super();
    this.type = type;
  ...
  public boolean equals(Object o) {
    if (!(o instanceof TestCaseParameter)) return false;
    TestCaseParameter tcp = (TestCaseParameter) o;
    if (tcp.getId() != null && this.getId() != null && tcp.getId().intValue() == this.getId())
      return true;

    if (tcp.getType() != this.getType()) return false;
    if (tcp.getNotification() != this.getNotification()) return false;
    if (tcp.getUnittypeParameter().getId() != this.getUnittypeParameter().getId().intValue())
      return false;
    if (tcp.getValue() == null && this.getValue() != null
        || tcp.getValue() != null && this.getValue() == null) return false;
    if (tcp.getValue() != null && !tcp.getValue().equals(this.getValue())) return false;
    return true;
  }

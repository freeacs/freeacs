...
public class Trigger {
  ...
  public static int getNotifyType(String notifyTypeStr) {
    for (Map.Entry<Integer, String> entry : notifyTypeMap.entrySet()) {
      if (entry.getValue().equals(notifyTypeStr)) return entry.getKey();
    }
  ...
  public static int getTriggerType(String triggerTypeStr) {
    for (Map.Entry<Integer, String> entry : triggerTypeMap.entrySet()) {
      if (entry.getValue().equals(triggerTypeStr)) return entry.getKey();
    }
    throw new IllegalArgumentException(
  ...
  private SyslogEvent
      syslogEvent; // Cannot be NULL for BASIC trigger type, otherwise NULL is possible

  private Integer noEvents; // No of trigger events required. Valid range: >0 (or NULL)
  private Integer
      noEventsPrUnit; // No of trigger event pr units required. Valid range: >0 (or NULL)
  ...
  // to ensure that the object is valid. The reason for requiring these two arguments, is that
  // several
  ...
  public void setNoUnits(Integer noUnits) {
    if (noUnits == null) {
      ...
      if (this.triggerType == TRIGGER_TYPE_COMPOSITE)
        throw new IllegalArgumentException(
            "Trigger noEventsPrUnit must be NULL if trigger type is COMPOSITE");
      if (noUnits < 1) throw new IllegalArgumentException("Trigger noUnits cannot be lower than 1");
      this.noUnits = noUnits;
    }
  }

  public void setUnittype(Unittype unittype) {
    if (unittype == null) throw new IllegalArgumentException("Trigger Unit Type cannot be null");
  ...
  protected void addChild(Trigger child) {
    if (children == null) children = new ArrayList<Trigger>();
